...
function updateTrails(dt) {
  ...
  for (let i = 0; i < bodies.length; i++) {
    ...
    // 3. ハイブリッドデータ構造への現在位置 ＆ 👑【最重要】離心率カラーの格納
    if (!b.trail) b.trail = [];
    ...
    b.trail.push({
      ...
      // 👑【バグ修正】その瞬間の離心率カラー(b.eccColor)を確実に配列へ保持！
...
function drawAngularMomentumVectorDirect2D(b, sun, pr, hideText = false) {
  ...
  // 👑【バグパージ】loop()側で毎フレーム更新されている最新の b.eccColor を直接参照！
  const currentVectorColor = b.eccColor || "#00a2ff";

  // レンダリング執行
  ctx.save();
  ctx.strokeStyle = currentVectorColor; // 針の色を天体本体の色と100%同期
  ctx.lineWidth = 0.8;
...
/* =======================================================
   ...
   ======================================================= */
/* =======================================================
   👑 軌跡描画エンジン（リアルタイム離心率カラー完全同調 ＆ 130-199 軌跡長動的最適化版）
   ======================================================= */
/* =======================================================
   👑【2】軌跡描画エンジン（130-199 軌跡長最適化 ＆ 離心率カラー完全追従版）
   ======================================================= */
/**
 * 👑 軌跡描画エンジン（線分単位グラデーション変色完全対応版）
 */
/**
 * 👑 軌跡描画エンジン（1セグメント完全分離・グラデーション変色 ＆ 極細ネオン完全版）
 */
/**
 * 👑 軌跡描画エンジン（1セグメント完全独立・太陽軌跡対応完全版）
 */
function drawBodyTrails(b, trailColor) {
  ...
  // 🛡️ 1. 太陽オブジェクトの堅牢な判定（大文字・小文字・typeの全網羅）
  const isSunObj = (b.name === "Sun" || b.name === "sun" || b.type === "sun" || b.type === "Sun");

  // 🛡️ 2. UI表示フラグの抽出（太陽フラグ未定義時はデフォルトで表示許可）
  ...
  for (let i = 0; i < totalPoints; i++) {
    if (i < firstIndex) { projectedTrail.push(null); continue; }
    const p = b.trail[i];
    ...
    if (prp.visible && !isNaN(prp.x) && !isNaN(prp.y)) {
      // ☀️ 太陽の場合は美しい白光色、それ以外は記録された離心率カラーを採用
      const pointColor = isSun ? "rgba(255, 255, 255, 0.95)" : (p.color || b.eccColor || trailColor || "#00a2ff");
      projectedTrail.push({ x: prp.x, y: prp.y, color: pointColor });
    } else {
  ...
  // 👑【1セグメント完全独立描画ループ】
  for (let i = firstIndex; i < totalPoints - 1; i++) {
    const p1 = projectedTrail[i];
    const p2 = projectedTrail[i + 1];
 ...
 */
/**
 * 天体コア描画エンジン（離心率カラー最優先・完全リアルタイム同期版）
 */
function drawBodyCore(b, pr, sun, screenSize) {
  ...
  if (isSun) {
    ...
    // 👑【最重要修正】b.eccColor を何よりも最優先で評価！
    // リアルタイム離心率カラーが存在すればそれを採用し、固定色(b.color)による上書きを阻止する
...
// =====================================================================
// 👑【バグパージ】カメラ状態保持変数の安全グローバル宣言
// loop() が参照する前に、未定義エラー（ReferenceError）を完全に遮断する
// =====================================================================
if (typeof lastCamRotX === "undefined") var lastCamRotX = 0;
if (typeof lastCamRotY === "undefined") var lastCamRotY = 0;
if (typeof lastCamZoom === "undefined") var lastCamZoom = 0;
if (typeof lastCamOffsetX === "undefined") var lastCamOffsetX = 0;
if (typeof lastCamOffsetY === "undefined") var lastCamOffsetY = 0;
if (typeof cameraChanged === "undefined") var cameraChanged = false;
...
function updateBodyEccentricityColor(b, sun) {
  ...
  // 👑 離心率判定マトリクス（リアルタイム変色）
  if (ecc < 0.08)      b.eccColor = "#00a2ff"; // 完全円軌道（シアン・青）
  else if (ecc < 0.25) b.eccColor = "#00e58b"; // 微小楕円（エメラルド・緑）
  else if (ecc < 0.45) b.eccColor = "#ded000"; // 楕円軌道（イエロー）
  else if (ecc < 0.75) b.eccColor = "#ff7700"; // 偏平軌道（オレンジ）
  else                 b.eccColor = "#ff2a55"; // 極限軌道・放物線（マゼンタレッド）
}
...
/* =========================================================
   ...
   ========================================================= */
/* =========================================================
   👑【1】軌跡カラー取得エンジン（完全同調・安全判定版）
   ========================================================= */
function getTrailColor(b, sun) {
  if (!b) return "rgba(255, 255, 255, 0.5)";

  // 1. UIで明示的に "white" が指定されている場合のみ白色を返却
  if (typeof settings !== "undefined" && settings.trailColorMode === "white") {
    return "#ffffff";
  }

  // 2. 👑 天体本体のリアルタイム離心率カラー(b.eccColor)を絶対最優先で返却！
  // 白色の自動生成計算を廃止し、本体色との100%完全同期を保証する
  ...
  // 💡 中心(t=0)で白熱(255, 255, 255)、遠方(t=1)で冷徹シアン(180, 220, 255)へ遷移
  const rCol = 255 - (255 - 180) * t;
  const gCol = 255 - (255 - 220) * t;
  const bCol = 255;
