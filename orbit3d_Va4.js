...
const settings = {
  ...
  // ■ 演算閾値・ハードウェア保護 (互換性を保つため両記法を保持)
  nbodyThreshold: 200,            // 全重力閾値 (この数値以下でN-body相互干渉が発動)
  fullGravityThreshold: 200,      // 旧コード参照用互換エイリアス
...
const simulationState = {
  ...
  camera: {
    ...
    followMode: "Sun",            // 追尾モード (★新設補填: "Sun" | "Center")
    autoRotate: true,             // 自動カメラ旋回有効フラグ (★新設補填)
    rotateSpeed: 0.001            // 自動カメラ旋回角速度 (★新設補填)
  },
...
const DEBUG_PRESETS = {
 // 💥 1. 重力カオス・スペクタクル（「動」の宇宙・199天体リアルタイムN-body）
    preset1: {
        ...
        // ---------------------------------------------------------------------
        // 1. UI入力項目・表示コントロール値 (最新設定構造準拠)
        // ---------------------------------------------------------------------
        ui: {
            // ■ 配置天体数量 (199基の超過密・リアルタイムN-body群)
            bodyCount: 199,
            bodyCountInput: 199,
            ...
            objMass: 1.0,
            objMassInput: 1.0,
            useFixedObjMass: false,    // false = ランダム質量 (M-RND)

            // ■ 力学・物理定数
            gravitySlider: 1.2,        // 相互作用が美しく出る重力倍率 (1.2)
            gravityInput: 1.2,
            spawnVelSlider: 1.0,       // 標準初速 (1.0)
            spawnVelInput: 1.0,
            dragSlider: 0.0,           // 空間抵抗 (0.0 = ブレーキなし)
            dragInput: 0.0,
            nbodyBoostSlider: 1.0,     // 重力ブースト (1.0)
            nbodyBoostInput: 1.0,
            speedSlider: 1.5,          // クロック速度 (1.5)
            speedInput: 1.5,

            // ■ 初期配置境界 (RANGE / spawnSettings)
            spawnMinMass: 10.0,
            spawnMaxMass: 100.0,
            spawnMinDist: 300,
            spawnMaxDist: 1200,
            spawnDirection: "chaos",   // 標準 (10% Retrograde)

            // ■ 演算閾値・ブレーキ解除制御
            nbodyThreshold: 3000,      // ★ ブレーキ解除: 3000 (199天体すべてが最初から相互干渉)
            nbodyLimitInput: 300,      // ★ 計算上限拡大: 300 (全天体の相互計算を許可)
            eps2Input: 30,             // 軟化パラメータ (30)

            // ■ 描画・ビジュアル
            trailLenSlider: 0.5,
            trailColorMode: "pure",    // 天体固有色

            // ■ カメラ・ターゲット
            cameraRotateSpeed: 0.0008, // 緩やかな自動カメラ旋回 (0.0008)
            targetInput: "AUTO",
            followMode: "Sun",
        ...
        // ---------------------------------------------------------------------
        // 2. 物理エンジン・システム内部変数・ボタン点灯同期 (実行ロジック)
        // ---------------------------------------------------------------------
        physics: function() {
            const u = this.ui; // ★ 標準関数メソッド化により this.ui を安全にキャプチャ

            // A. グローバル変数への代入
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = u.bodyCount;
            if (window.bodies) window.bodies.length = u.bodyCount; // 配列長の自動調整

            // B. 物理設定オブジェクト (settings) への完全注入 (新旧互換構造)
            ...
            // C. シミュレーション状態 (simulationState) へのカメラ・UIフラグ注入
  ...
  // 🪐 2. 双太陽・二重連星系（「連」の宇宙）
    preset2: {
        ...
        // ---------------------------------------------------------------------
        // 1. UI入力項目・表示コントロール値 (既存数値を100%維持)
        // ---------------------------------------------------------------------
        ui: {
            // ■ 配置天体数量 (連星軌道が見やすい適正数量: 32天体)
            bodyCount: 32,
            bodyCountInput: 32,
            ...
            objMass: 2.0,
            objMassInput: 2.0,
            useFixedObjMass: "RND",    // 質量適用モード ("RND" / "FIX")

            // ■ 力学・物理定数
            gravitySlider: 1.2,        // 滑らかな連続偏向を生む重力 (1.2)
            gravityInput: 1.2,
            spawnVelSlider: 0.9,       // 軌道捕獲率を高めるしっとりした速度 (0.9)
            spawnVelInput: 0.9,
            dragSlider: 0.0,           // 空間抵抗 (0.0)
            dragInput: 0.0,
            nbodyBoostSlider: 1.0,     // 重力ブースト (1.0)
            nbodyBoostInput: 1.0,
            speedSlider: 2.5,          // 軌道幾何学を観察できる速度 (2.5)
            speedInput: 2.5,
            ...
            // ■ 演算閾値・数値制御
            nbodyThreshold: 200,       // 全重力閾値
            nbodyLimitInput: 150,      // 演算上限
            eps2Input: 20,             // 軟化係数 (20)
            ...
            // ■ カメラ・ターゲット
            cameraRotateSpeed: 0.0003, // 重心運動を引き立てるスロー視点 (0.0003)
            targetInput: "AUTO",
            followMode: "Sun",

            // ■ Goliath 召喚用 INJECT 初期セット
            injectType: "sun2",        // 第二の太陽 (Sun2)
            injectModeSelect: "normal",
            injectMassSlider: 1500,    // Goliath 質量 1500
        ...
        // ---------------------------------------------------------------------
        physics: function() {
            const u = this.ui; // ★ 最新フォーマットのスコープ参照

            // A. グローバル変数および物理設定オブジェクト (settings) への一括注入
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = u.bodyCount;
            if (typeof window.settings !== "undefined") {
                settings.sunInitialVx = u.sunVxInput;
                settings.sunInitialVy = u.sunVyInput;
                ...
                settings.eps2 = u.eps2Input;
                settings.objBaseMass = u.objMassInput;
                settings.useFixedObjMass = (u.useFixedObjMass === "FIX");
                settings.sunFixed = false;
                settings.trailColorMode = u.trailColorMode;
                settings.drag = u.dragInput;
                settings.nbodyBoost = u.nbodyBoostInput;
            ...
            // D. トグルボタンの LED 点灯クラス一括制御 (惑星軌跡・重心線・太陽軌跡をON)
  ...
  // 📐 3. ケプラー解析・物理デコーダー（「知」の宇宙）
    preset3: {
        ...
        ui: {
            ...
            // ■ 力学・物理定数
            gravitySlider: 1.0,        // 標準重力定数 (1.0)
            gravityInput: 1.0,
            spawnVelSlider: 0.82,      // 綺麗な高偏平楕円 (0.82)
            spawnVelInput: 0.82,
            dragSlider: 0.0,           // 空間抵抗 (0.0)
            dragInput: 0.0,
            nbodyBoostSlider: 1.0,     // 重力ブースト (1.0)
            nbodyBoostInput: 1.0,
            speedSlider: 2.0,          // 解析用クロック (2.0)
            speedInput: 2.0,
            ...
            // ■ カメラ・ターゲット
            cameraRotateSpeed: 0.0002, // 超微速旋回 (0.0002)
            targetInput: "AUTO",
            followMode: "Sun",
        ...
        physics: function() {
            ...
            // C. 特有設定: L-VEC モードのセット (角運動量・離心率解析)
            window.lVecMode = 2;
...
// 🌌 4. 3000天体 スムース・ロードシム（初期設定完全準拠・動的LOD連動）
    preset4: {
        name: "PRST: 4 (3000天体 スムースシム)",

        // ---------------------------------------------------------------------
        // 1. UI入力項目・表示コントロール値 (初期リロード設定 100% 準拠)
        // ---------------------------------------------------------------------
        ui: {
            // ■ 配置天体数量 (3000天体仕様)
            bodyCount: 3000,
            ...
            gravitySlider: 1.0,
            gravityInput: 1.0,
            spawnVelSlider: 0.95,      // 0.95 (黄金比初速)
            spawnVelInput: 0.95,
            dragSlider: 0.0,           // 空間抵抗 (0.0)
            dragInput: 0.0,
            nbodyBoostSlider: 1.0,     // 重力ブースト (1.0)
            nbodyBoostInput: 1.0,
            speedSlider: 2.5,          // 演算速度 (2.5)
            speedInput: 2.5,
            ...
            // ■ 演算閾値・数値制御 (初期設定通り)
            nbodyThreshold: 200,       // 全重力閾値 (200以下でN-body全相互作用へ昇格)
            nbodyLimitInput: 150,      // 演算上限ノード数 (150)
            eps2Input: 36,             // 軟化係数 (36)

            // ■ 描画・ビジュアル
            trailLenSlider: 1.2,       // 1.2
            trailColorMode: "pure",    // 天体固有色

            // ■ カメラ・ターゲット
            cameraRotateSpeed: 0.001,  // カメラ旋回速度 (0.001)
            targetInput: "AUTO",
            followMode: "Sun",
        ...
        physics: function() {
            ...
            // B. シミュレーション状態 (simulationState) へのフラグ注入 (初期設定準拠)
            if (typeof window.simulationState !== "undefined") {
                if (simulationState.ui) {
                    simulationState.ui.showPlanetTrail = true;   // 惑星軌跡 ON
                    simulationState.ui.showCometTrail  = true;   // 彗星軌跡 ON
                    simulationState.ui.showSunTrail    = false;  // 太陽軌跡 OFF
                    simulationState.ui.showNames       = false;  // 名前ラベル OFF
                    simulationState.ui.showLVec        = 0;      // L-VEC OFF
...
// ========================================================
// 🛸 神の意志を無視する：Goliath / Obj 動的強制生成コマンド (強化版)
...
function spawnGoliathForce() {
    ...
    const elType = document.getElementById("injectType");
    const elMassInput = document.getElementById("injectMassInput");
    const elMode = document.getElementById("injectModeSelect"); // 🎯【拡張】軌道モードUI（任意）
        const selectedType = elType ? elType.value : "obj";
    const orbitMode = elMode ? elMode.value : "normal"; // "normal", "retro" (逆行), "infall" (直撃), "polar" (極軌道)
        let customMass = elMassInput ? parseFloat(elMassInput.value) : 5.0;
    if (isNaN(customMass) || customMass <= 0) customMass = 5.0;

    // 1. 【3次元幾何学配置】（黄道面への適度な収束オプション）
    const angle1 = Math.random() * Math.PI * 2;
    // 極軌道モード以外は、黄道面（z=0）付近へ少し引きつける
    ...
    const gravityMult = settings?.gravityMultiplier || 1.0;
        // システム内の全質量を考慮した実効ポテンシャル速度（多重重力場補正）
    ...
    if (orbitMode === "infall") {
        ...
        // 【逆行モード】：時計回りに暴走（既存天体と正面衝突コース）
        vx = (y / dist) * v;
        vy = (-x / dist) * v;
        vz = (Math.random() - 0.5) * 0.2 * v;
    } else {
        // 【通常・順行モード】：反時計回りの安定公転
        const side = (Math.random() < 0.05) ? -1 : 1; // 5%の確率で逆流
        vx = (side * -y / dist) * v;
        vy = (side * x / dist) * v;
...
// ---------------------------------------------------------
  // ② カメラ自動巡航 UIコントロール初期化（完全同期・バグ修復版）
  // ---------------------------------------------------------
  ...
  function syncCameraRotateSpeed(value, isFromInput = false) {
    ...
    // 1. 全ての関連速度変数へ同時に代入（変数名の表記ブレを完全吸収）
  ...
  if (camSpeedInput) {
    ...
    // フォーカスが外れた時（クランプした値を確実に変数とUIへ反映）
...
// ========================================================
// 🛡️ 199天体 相転移・動的エッジ検出器 (メインループ内へ配置)
// ========================================================

// グローバルまたはシミュレーション状態管理変数
...
function checkPhaseTransitionThreshold() {
    ...
    // 1. 物理的に「生存している天体」の正確なカウント（論理削除対応）
    const activeBodies = bodies.filter(b => b && (b.alive !== false) && !b.isDead);
    const currentAliveCount = activeBodies.length;
    ...
    if (currentAliveCount <= 199 && !window.isPhaseTransitionTriggered) {
        ...
        if (typeof enablePhaseTransition === "function") {
            enablePhaseTransition();
                ...
                // L-VEC や UI 側の表示更新トリガー
        const lvecBtn = document.getElementById("toggle-momentum-btn");
        if (lvecBtn) {
            lvecBtn.style.display = "inline-block"; // 非表示だったボタンを出現させる等
        }
    }
...
// =======================================================
 ...
 // -------------------------------------------------------
  ...
  if (typeof detectCollisionsWithGrid === "function") {
    ...
    for (let k = collisions.length - 1; k >= 0; k--) {
      ...
      const newBody = {
        ...
        // 👑【修正版：M^0.30 モデル】500以上でもダイナミックに成長を視認可能！
   ...
   if (isSunCollision) {
        // 👑【100%確実発火】外部関数に頼らずその場で直接ログ配列を作成・更新する
        ...
        if (targetObj) {
          ...
          if (window.sunEventLogs.length > 5) {
            window.sunEventLogs.shift(); // 直近5件を保持
          }
        }
  ...
  for (let i = bodies.length - 1; i >= 0; i--) {
    const b = bodies[i];
    if (b.name === "Sun" || b.type === "sun") continue; // 太陽は絶対にパージしない
...
/* =========================================================
   1. メイン描画コントロール（500個刻み動的LOD ＆ ゼロ・GC最適化版）
   ========================================================= */

// ★ 描画用Zソートバッファの事前確保（毎フレームの配列生成をゼロにして高速化）
...
function renderScene() {
  ...
  // ---------------------------------------------------------
  // LOD 0: 3000~2500 (超高密度・粒子雲)
  // LOD 1: 2500~2000 (ベクトル電界)
  // LOD 2: 2000~1500 (彗星流展開)
  // LOD 3: 1500~1000 (ショートトレイル)
  // LOD 4: 1000以下   (完全精度・フル描画)
  let lodLevel = 4;
  ...
  // 手動で極小モードが指定されている場合は LOD 0 へ強制
  if (simulationState?.ui?.vectorFieldOnly) lodLevel = 0;

  const hideVectorText = lodLevel <= 1; // LOD 0, 1 ではベクトルのテキスト表記を非表示

  // ---------------------------------------------------------
  // 🚀 Z深度ソートの高速化（新規配列を作らずバッファを再利用）
  // ---------------------------------------------------------
  if (renderSortBuffer.length !== totalCount) {
    renderSortBuffer = new Array(totalCount);
  }
  for (let i = 0; i < totalCount; i++) {
    const b = bodies[i];
    renderSortBuffer[i] = { b, r: rotate3D(b) };
  }
  ...
  for (let i = 0; i < totalCount; i++) {
    ...
    // 1. 軌跡（Trail）描画: 太陽は常に描画、小天体は LOD 3 (1500以下) から解禁
    if (isSunObj || lodLevel >= 3) {
      drawBodyTrails(obj.b, trailColor);
    }

    // 2. 彗星の尾（Comet Tail）描画: LOD 2 (2000以下) から解禁
    if (!isSunObj && lodLevel >= 2) {
  ...
  // ---------------------------------------------------------
  // 【第4階層】システムHUD（ソート済みバッファを渡す）
  // ---------------------------------------------------------
  drawGravityCenterOfTop2(renderSortBuffer);
...
function drawAngularMomentumVectorDirect2D(b, sun, pr, hideText = false) {
  ...
  // 🚀【超高速化】始点投影（pBase）は引数の pr をそのまま使用！先端（vRot）のみ1回だけ3D投影する
  ...
  // 引数 pr（天体本体の画面座標）を始点とする
  const startX = pr.x;
  const startY = pr.y;

  // 重畳計算していた pBase 依存を無くし、直接先端のスクリーンスロープを割り出す
  // (※ pTip と 天体投影位置の差分を先端へ加算)
  const bRot = (typeof rotate3D === "function") ? rotate3D({ x: b.x, y: b.y, z: b.z }) : { x: b.x, y: b.y, z: b.z };
  const pBaseCalculated = (typeof project3D === "function") ? project3D(bRot.x, bRot.y, bRot.z) : { x: pr.x, y: pr.y };

  const endX = startX + (pTip.x - pBaseCalculated.x);
  const endY = startY + (pTip.y - pBaseCalculated.y);

  // 💡 離心率(e)のリアルタイム計算（物理定数同期）
  let ecc = 0;
...
/* =======================================================
   👑 軌跡描画エンジン（300〜100間 1/3短縮＆100以下フル解禁版）
   ======================================================= */
/* =======================================================
   👑 軌跡描画エンジン（2500以下完全同期・多段スケーリング修正版）
   ======================================================= */
/* =======================================================
   👑 軌跡描画エンジン（閾値200最適化・軽量シームレス版）
   ======================================================= */
/* =======================================================
   👑 軌跡描画エンジン（199-130 軌跡長動的最適化版）
   ======================================================= */
function drawBodyTrails(b, trailColor) {
  ...
  const zoom = (typeof camera !== "undefined" && camera.zoom) ? camera.zoom : 1.0;
  let baseWidth = 0.15;
  if (isPlanet) baseWidth = (b.isOrbiting ? 0.4 : 0.25) * (0.2 + zoom * 0.1);
  else if (isComet) baseWidth = 0.08;
  else if (isSun) baseWidth = 0.5;

  const totalCount = (typeof bodies !== "undefined") ? bodies.length : 100;

  // 🚀【199 ➔ 130 軌跡長最適化アルゴリズム】
  ...
  } else if (totalCount >= 130 && totalCount <= 199) {
    // 💡 199〜130個の間: 35% ➔ 80% へ無段階・スムーズに自動伸長！
    const u = (199 - totalCount) / (199 - 130); // 進行度 (0.0 〜 1.0)
    const ratio = 0.35 + u * 0.45;              // 0.35 (35%) ➔ 0.80 (80%)
    effectivePoints = Math.max(5, Math.floor(totalPoints * ratio));

  } else if (totalCount > 199 && totalCount <= 2500) {
    // 199超〜2500個: 20% ➔ 35% スケーリング
    const ratio = 0.20 + (1.0 - (totalCount - 199) / 2301) * 0.15;
    effectivePoints = Math.max(5, Math.floor(totalPoints * ratio));

  } else {
    // 2500個超
    effectivePoints = Math.max(4, Math.floor(totalPoints * 0.15));
  }
  ...
  // 3. ステップ数の最適化配置
  let desiredSteps;
  if (totalCount <= 199)      desiredSteps = effectivePoints - 1;                // 199以下: 100%全点完全分解描画
  else if (totalCount <= 500) desiredSteps = Math.min(15, effectivePoints - 1);
  else                        desiredSteps = Math.min(8,  effectivePoints - 1);

  const numSteps = Math.min(desiredSteps, Math.max(1, effectivePoints - 1));

  // 投影座標の事前抽出
  const projectedTrail = [];
  for (let i = 0; i < totalPoints; i++) {
    if (i < firstIndex) {
      projectedTrail.push(null);
      continue;
    }
    const p = b.trail[i];
    if (!p) { projectedTrail.push(null); continue; }
  ...
  for (let step = 0; step < numSteps; step++) {
    const startIndex = firstIndex + Math.floor((step * (effectivePoints - 1)) / numSteps);
    const endIndex   = Math.min(totalPoints - 1, firstIndex + Math.floor(((step + 1) * (effectivePoints - 1)) / numSteps) + 1);

    if (startIndex >= endIndex) continue;

    let validP1 = null, validP2 = null;
    for (let i = startIndex; i < endIndex; i++) {
      if (projectedTrail[i] && projectedTrail[i + 1]) {
        validP1 = projectedTrail[i];
        validP2 = projectedTrail[i + 1];
        break;
      }
    }
    if (!validP1 || !validP2) continue;

    // 進行度 t (0.0 〜 1.0)
    const midPoint = (startIndex + endIndex) / 2;
    const t = (midPoint - firstIndex) / Math.max(1, effectivePoints - 1);
    ...
    // 1段階目のソフト上限 (0.85)
    const maxAlpha = 0.85;
    ctx.globalAlpha = smoothT * maxAlpha;

    ctx.lineWidth = baseWidth * (0.2 + 0.8 * smoothT);

    ctx.beginPath();
    ctx.moveTo(validP1.x, validP1.y);

    for (let i = startIndex + 1; i <= endIndex; i++) {
      const pt = projectedTrail[i];
      if (pt) {
 ...
 * 👑【演出100%保持 ＆ あいまい名前判定・ゴリアテサイズ適正化版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【Obj最大サイズ ＝ 太陽同等クランプ ＆ 完全防空版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【太陽＆一般天体 全スケール最適化 ＆ 完全防空版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【カメラ距離不依存・画面比率上限ガード ＆ 完全防空版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【Obj最大値 ＝ ゴリアテ最大値同等化 ＆ 完全スケール調和版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【ズームアウト完全遠近追従 ＆ 太陽・一般天体ジャストサイズ版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【SUN ＆ 合体Obj ズームアウト完全連動・動的スケーリング版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【太陽の初期存在感保証 ＆ 全天体スムーズ・ズームスケーリング版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【Injectゴリアテ最低サイズ保証 ＆ SUN・全天体最適スケーリング版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【案1：3乗根モデル適用 ＆ Goliath・Obj 完全同一スケール統合版】
 */
/**
 * 天体本体の球体を描画する
 * 👑【100〜3000 質量比較・完全可視化スケーリング版】
 */
/**
 * 天体コア描画エンジン（3乗根質量スケール ＆ 離心率カラー完全同調版）
 */
function drawBodyCore(b, pr, sun, screenSize) {
  ...
  // 💡 1. 太陽の基本スケール
  const sunScaleFactor = 0.09;
  let rawBaseSize = 1.0;

  if (isSun) {
    // ☀️ 太陽：最低 2.5 を保証した独立スケーリング
    const sunRaw = b.renderSize ?? b.size ?? b.mass ?? 1000.0;
    rawBaseSize = Math.max(2.5, sunRaw * sunScaleFactor);
  } else {
    // 🪐 👾 一般天体 ＆ ゴリアテ（3乗根モデルによる絶対比較）
    const mass = Number(b.mass ?? b.m ?? b.renderSize ?? b.size ?? 1.0);

    const M_MIN = 100.0;   // 変化の始点質量
    const M_MAX = 3000.0;  // 変化の終点質量

    // 💡 質量 100 ➔ 3000 で明確な差が出る可視範囲（1.2px 〜 4.8px）
    const R_MIN = 1.2;  // M <= 100 のときの小さな点
    const R_MAX = 4.8;  // M >= 3000 のときの存在感あるサイズ（ゴリアテ最適値）
    ...
    // 1.2 から 4.8 へスムーズに連続拡大
    rawBaseSize = R_MIN + (R_MAX - R_MIN) * cubicGrowth;
  }

  // 🚀 2. 画面占有率上限ガード（近接ズーム時のみ作動）
  const screenHeight = ctx.canvas ? ctx.canvas.height : 1000;
  const maxPixelRadius = isSun ? (screenHeight * 0.022) : (screenHeight * 0.012);

  const calculatedRadius = rawBaseSize * screenSize;
  const finalRadius = Math.max(1.0, Math.min(maxPixelRadius, calculatedRadius));
  ...
  if (isSun) {
    const grad = ctx.createRadialGradient(pr.x, pr.y, finalRadius * 0.1, pr.x, pr.y, finalRadius);
    ...
    const grad = ctx.createRadialGradient(pr.x, pr.y, finalRadius * 0.2, pr.x, pr.y, finalRadius);
    ...
    // 👑【物理同調】針の色（b.eccColor: 離心率カラー）を最優先で本体色に適用！
    ctx.fillStyle = b.eccColor ||
                    ((typeof getCelestialColor === "function") ? getCelestialColor(b, sun) : (b.color || "#00a2ff"));
...
// ------------------------------------------
  // 📡 ブロック6：【SUN吸収イベントログ（直近5件）】
  // ------------------------------------------
  if (window.sunEventLogs && window.sunEventLogs.length > 0) {
    // 💡 1. REALテキストからしっかり下に余白を取る
    currentY += Math.round(lineHeight * 0.8);
    ...
    ctx.lineTo(pX + 380, currentY); // 横幅を少し広げて美しく拡張
    ctx.stroke();

    // 💡 3. 線を描いた後、ログ1行目をさらに綺麗に「下へ押し下げる」
    currentY += Math.round(lineHeight * 0.9);

    ctx.fillStyle = "#00ffcc"; // 吸収ログ専用カラー（サイバーシアン）
    for (let i = 0; i < window.sunEventLogs.length; i++) {
      ctx.fillText(window.sunEventLogs[i], pX, currentY);
      currentY += lineHeight;
    }
  }
