...
// 👑 3D 6 軸ギズモ（直角針）の表示フラグ (初期値: OFF)
let showAxesGizmo = false;
/* =========================================================
...
// 🪐 2. 双太陽・二重連星系（「連」の宇宙 - パラメータ解説付き完全版）
  preset2: {
    name: "PRST: 2 (双太陽・二重連星系)",

    // ------------------------------------------------------------------------
    // 🎛️ UI & 初期値設定プロパティ (数値チューニング領域)
    // ------------------------------------------------------------------------
    ui: {
      // 🌌 [天体数] 画面上に生成する小天体の総数 (多いと重力井戸のメッシュが密になるが計算負荷増)
      bodyCount: 1999,
      bodyCountInput: 1999,

      // ☀️ [主太陽 (SUN1) 質量] 中心に存在する主星の重力ポテンシャル強度
      sunMass: 2500,
      sunMassInput: 2500,
            // 🚀 [主太陽 初速] 主星の初期移動速度ベクトル (基本は 0 で静止)
      sunVxSlider: 0.0, sunVxInput: 0.0,
      sunVySlider: 0.0, sunVyInput: 0.0,
      sunVzSlider: 0.0, sunVzInput: 0.0,

      // 🪨 [小天体 ベース質量] 標準となる小天体の質量 (ギズモの標準半径 radius に影響)
      objMass: 15.0,
      objMassInput: 15.0,
      useFixedObjMass: false, // true: 全天体を固定質量にする / false: ランダム領域を適用

      // 🌌 [万有引力係数 (G)] 空間全体の重力強度の倍率 (大きくすると運動が高速化・急激化)
      gravitySlider: 1.2,
      gravityInput: 1.2,

      // 💫 [初速倍率] 天体スポーン時に与えられる初期公転速度の倍率 (小さいと落下、大きいと離散)
      spawnVelSlider: 0.85,
      spawnVelInput: 0.85,

      // 🛑 [空間抵抗 (Drag)] 運動エネルギーの減衰率 (0.0: 無抵抗の宇宙空間 / 大: 粘性抵抗)
      dragSlider: 0.0,
      dragInput: 0.0,

      // ⚡ [N-Body ブースト] 天体相互の引力倍率 (1.0: 標準 / 大: 天体同士の引き合い・弾き合いを強調)
      nbodyBoostSlider: 1.0,
      nbodyBoostInput: 1.0,

      // ⏩ [シミュレーション速度] 1フレームあたりの時間刻み (Delta-t) の進み幅
      speedSlider: 2.2,
      speedInput: 2.2,

      // ⚖️ [スポーン質量範囲] ランダム生成される天体の最小・最大質量 (ギズモ描画閾値に直結)
      spawnMinMass: 10.0, // 最小質量 (大きいほど初手からウニ/ギズモが確実に描画される)
      spawnMaxMass: 40.0, // 最大質量

      // 📐 [スポーン距離範囲] 主星からの生成距離 (Min: 連星近傍 / Max: 外縁部)
      spawnMinDist: 250,
      spawnMaxDist: 1500,

      // 🌀 [スポーン配置幾何] "disk": 赤道面円盤状(時空シート) / "chaos": 球状ランダム
      spawnDirection: "disk",

      // 🧮 [N-Body 計算制御] 近接 N-Body 相互作用を全計算する近傍距離と上限天体数
      nbodyThreshold: 300,   // N-Body 計算対象とする最大距離
      nbodyLimitInput: 200,   // 相互計算を行う最大天体数 (パフォーマンス保護)

      // 🛡️ [イプシロン自乗 (eps^2)] 重力計算の軟化長 (0に近づけると超近接時に速度無限大で跳ね飛ぶ)
      eps2Input: 20,

      // 🎨 [軌跡・カメラ演出]
      trailLenSlider: 0.1,      // 軌跡の残像長 (0.1: 極薄残像でギズモ線を強調 / 1.0: 長い軌跡)
      trailColorMode: "pure",   // 軌跡の色彩モード

      cameraRotateSpeed: 0.0004, // 自動カメラ旋回速度 (極低速回転で 3D 深度を補完)
      targetInput: "AUTO",      // カメラの注視点ターゲット
      followMode: "Sun",        // 追従モード

      // 🪐 [第二太陽 (Goliath / SUN2) 注入設定]
      injectType: "sun2",        // 注入するオブジェクト種別 ("sun2": 第二太陽)
      injectModeSelect: "normal",
      injectMassSlider: 1500,    // 第二太陽の質量 (主星 2500 に対し 1500 の二重連星系を形成)
    ...
    physics: function() {
      ...
      // 1. システム設定オブジェクト (settings) へ UI パラメータを一括転送
      if (typeof initialBodyCount !== "undefined") window.initialBodyCount = u.bodyCount;
      if (typeof window.settings !== "undefined") {
        settings.sunInitialVx = u.sunVxInput;
        settings.sunInitialVy = u.sunVyInput;
      ...
      // 3. DOM エレメント (入力スライダー等) への値反映とイベント同期
      ...
      // 4. 第二太陽 (Goliath) の物理空間への生成執行
      if (typeof spawnGoliathForce === "function") {
        spawnGoliathForce();
      }

    // 5. 👑 Gボタン（ギズモ）の確定アクティブ化 (イベント強制発火)
      // 単なる変数代入ではなく、ボタンの click() を実行して内部ロジックを叩く
      const gizmoBtnIds = ['btnGizmo', 'toggleGizmoBtn', 'btnToggleGizmo', 'gizmoBtn'];
      let gizmoActivated = false;

      gizmoBtnIds.forEach(id => {
        const btn = document.getElementById(id);
        if (btn) {
          // ボタンが現在 OFF 状態（active クラスを持っていない）場合のみクリックを発火
          if (!btn.classList.contains('active') && !btn.classList.contains('toggle-on')) {
            btn.click(); // 👈 イベントハンドラを呼び出し、内部 State と描画処理を起動
          }
          gizmoActivated = true;
        }
      });

      // フォールバック: ボタン DOM が存在しない場合のみグローバルフラグを直接更新
      if (!gizmoActivated) {
        window.showGizmo = true;
        window.isGizmoEnabled = true;
      }

      // 6. その他の UI ボタン状態の制御
      const targetON  = ['toggleBaryBtn', 'sunTrailBtn', 'btnToggleRotate'];
      const targetOFF = ['planetTrailBtn', 'toggle-momentum-btn', 'cometTrailBtn', 'showNames', 'btnOrbitCam'];

      targetON.forEach(id => {
        const btn = document.getElementById(id);
        if (btn && !btn.classList.contains('active')) {
          btn.click(); // 他の ON ボタンも click() で統一発火
        }
      });
      ...
      targetOFF.forEach(id => {
        const btn = document.getElementById(id);
        if (btn && btn.classList.contains('active')) {
          btn.click(); // ON になっているものだけ click() して OFF に切替
        }
      });

      // 7. カメラ旋回速度の適用と DOM への同期完了
      if (typeof syncCameraRotateSpeed === "function") {
        syncCameraRotateSpeed(u.cameraRotateSpeed, true);
      }
      window.isAutoRotateEnabled = true;
      ...
      if (window.UI_DEBUG) {
        console.log("🪐 [PRST: 2] 描画・物理パラメータ設定の適用が正常完了しました。");
      }
    }
  },
...
// =========================================================
  // 👑【ここに追加 1】全天体描画ループに入る前に、太陽の投影座標 (window.prSun) を更新
  // =========================================================
  if (typeof project3D === "function") {
    // 太陽の 3D 座標を 2D スクリーン座標に変換
    const prTemp = project3D(sun.x, sun.y, sun.z, camera, W, H);
        if (prTemp) {
      // メモリ割当 (0 Byte) を維持するため、既存オブジェクトのプロパティのみを更新
      window.prSun = window.prSun || { x: 0, y: 0, depth: 0, visible: true };
      window.prSun.x = prTemp.x;
      window.prSun.y = prTemp.y;
      window.prSun.depth = prTemp.depth || 0;
      window.prSun.visible = prTemp.visible !== false;
    }
  }
...
// 👑【修正版】太陽以外の天体に「リアルタイム満ち欠け（陰影）」を重ね描き
if (!isSun && typeof renderBodyPhase === "function" && typeof camera !== "undefined" && window.prSun) {
  // 1. 極座標 (distance, elevation, azimuth) から 3D 直交座標 (x, y, z) をその場で動的変換
  const cDist = camera.distance || 2000;
  const cElev = camera.elevation || 0.523;
  const cAzim = camera.azimuth || 0;
  const sX = sun ? sun.x : 0;
  const sY = sun ? sun.y : 0;
  const sZ = sun ? sun.z : 0;

  // 2. 3D 空間上のカメラ絶対座標の算出 (代数演算のみ / 0 Byte)
  const camX = sX + cDist * Math.cos(cElev) * Math.sin(cAzim);
  const camY = sY + cDist * Math.sin(cElev);
  const camZ = sZ + cDist * Math.cos(cElev) * Math.cos(cAzim);

  // 3. 直交座標オブジェクトを渡して満ち欠け描画を発動
  renderBodyPhase(
    b,
    sun,
...
// 👑 4. 3D 6軸ギズモ（直角針）描画（L-VEC モード＆エネルギー連動版）
// 👑 3D 6軸ギズモ（直角針）描画（[G] ボタンと直結・L-VEC カラー自動連動）
if (showAxesGizmo && typeof drawBodyAxesGizmo === "function") {
  drawBodyAxesGizmo(b, pr, finalRadius, sun);
...
// 3. エンジン本体コード（★ここに貼ります！）
// ==========================================
// ⚠️ drawBodyCore の「外側」（下）に配置してください

/**
 * 👑【完全 Zero-Allocation】天体中心 3D 6 軸ギズモ（直角針）描画エンジン
 */
/**
 * 👑【完全 Zero-Allocation】エネルギー・L-VEC 連動型 3D 6軸ギズモ描画エンジン
 * 速度・エネルギー状態に応じて 6 本の針の長さと色彩がリアルタイムに伸縮・変化
 */
/**
 * 👑【重力場連動型】3D 6 軸 Local-Frame ギズモ
 * ワールド絶対軸を全廃し、「重力場ベクトル」と「軌道面」へ 6 軸を動的アライメント
 */
/**
 * 👑【洗練版】役割別カラーコード ＆ LOD 制御付き 3D 6 軸ギズモ
 * 重力軸・軌道面軸を色彩分離し、過密領域での視認性を完璧に保持 (0 Byte/frame)
 */
/**
 * 👑【重力井戸 (Gravity Well) 可視化統合版】3D 6 軸ギズモ
 * ポテンシャルの深さに比例したドロップラインを生成し、「重力の谷」を立体的に描写
 */
/**
 * 👑【完全修正版】ポテンシャル・ベースリング統合型 3D 重力井戸 ＆ ギズモ
 * 絶対 Z 軸依存を全廃し、軌道法線に沿ったドロップラインと谷底リングで「重力の井戸」を完全可視化
 */
/**
 * 👑【完全洗練版】3D 6 軸 Local-Frame ギズモ
 * ノイズとなるドロップラインを完全排除し、重力軸（金）と軌道面（青）の立体幾何のみを極小可視化
 */
/**
 * 👑【倍長スケール版】3D 6 軸 / 4 軸 Local-Frame ギズモ
 * 軸長を 2 倍に拡張し、重力場の傾斜線（ポテンシャルライン）の空間集約を極限まで強化
 */
/**
 * 👑【異方性物理テンソル版】3D 6 軸 / 4 軸 Local-Frame ギズモ
 * 各軸の長さを「重力加速度」「速度」「角運動量」の各物理量へ個別に一対一割り当て
 */
/**
 * 👑【完全煮詰め版】ポテンシャル・スペクトル ＆ 非対称ベクトル型 3D ギズモ
 * 重力井戸の深度 (1/r) に応じた色相励起と非対称軸長により空間の歪みを極限まで可視化
 */
/**
 * 👑【多体摂動・高周波応答版】3D 6 軸 Local-Frame ギズモ
 * SUN の中心力オーバーライドを抑制し、N-Body 合成加速度 (|a|) による「びくびく感」を完全復元
 */
/**
 * 👑【マスターピース完成版】Z-Depth 減衰 ＆ 幾何キャップ統合 3D ギズモ
 * カメラ深度による透明度衰退、重力ベクトルキャップ、多体摂動線幅励起を完全統合 (0 Byte/frame)
 */
function drawBodyAxesGizmo(b, pr, radius, sun) {
  if (!b || !pr || radius < 1.2 || !sun) return;

  // 🛡️ 1. 【Zero-Allocation】0 Byte キャッシュ
  if (!drawBodyAxesGizmo.temp) {
    drawBodyAxesGizmo.temp = { x: 0, y: 0, z: 0 };
  }
  const temp = drawBodyAxesGizmo.temp;

  // 2. 3D 距離 r と 重力軸 (L)
  let Lx = sun.x - b.x, Ly = sun.y - b.y, Lz = sun.z - b.z;
  const dist = Math.sqrt(Lx * Lx + Ly * Ly + Lz * Lz) || 1;
  Lx /= dist; Ly /= dist; Lz /= dist;

  // 3. 速度 (V) ＆ 軌道法線 (N) / 接線 (M)
  const vx = b.vx || 0, vy = b.vy || 0, vz = b.vz || 0;
  let Nx = Ly * vz - Lz * vy, Ny = Lz * vx - Lx * vz, Nz = Lx * vy - Ly * vx;
  let N_len = Math.sqrt(Nx * Nx + Ny * Ny + Nz * Nz);
  if (N_len === 0) { Nx = 0; Ny = 1; Nz = 0; N_len = 1; }
  Nx /= N_len; Ny /= N_len; Nz /= N_len;

  const Mx = Ly * Nz - Lz * Ny, My = Lz * Nx - Lx * Nz, Mz = Lx * Ny - Ly * Nx;

  // ⚡ 4. N-Body 合成加速度 |a|（多体摂動）の抽出
  const ax = b.ax || 0, ay = b.ay || 0, az = b.az || 0;
  const accel = Math.sqrt(ax * ax + ay * ay + az * az);

  // 🌌 5. 👑【Z-Depth Alpha Fading】カメラ距離に応じた空気感（透明度減衰）の計算
  // pr.z または b.z に基づく奥行き減衰ファクター (手前: 1.0 ~ 遠方: 0.2)
  const depthAlpha = Math.max(0.2, Math.min(1.0, 1500 / (Math.abs(pr.z || b.z) + 600)));

  // 🎨 6. 色彩と透明度の複合動的決定
  const speed = Math.sqrt(vx * vx + vy * vy + vz * vz);
  const depthFactor = Math.min(1.0, 300 / dist);

  let aGrav = (accel > 0.5 || depthFactor > 0.7) ? 0.95 : 0.75;
  let aNorm = (accel > 0.5 || depthFactor > 0.7) ? 0.85 : 0.55;
  let aTang = (accel > 0.5 || depthFactor > 0.7) ? 0.75 : 0.35;
  ...
  const colGrav = `rgba(255, 215, 80, ${aGrav.toFixed(2)})`;
  const colNorm = `rgba(0, 200, 255, ${aNorm.toFixed(2)})`;
  const colTang = `rgba(220, 220, 255, ${aTang.toFixed(2)})`;

  // 📏 7. 非対称ベクトルの動的長さ
  const L_grav_pos = Math.max(12, radius * 1.2 + Math.min(55, (5000 / dist) + accel * 20.0 + speed * 1.0));
  const L_grav_neg = L_grav_pos * 0.35;
  const L_norm = Math.max(8, radius * 1.1 + Math.min(30, N_len * 0.1 + accel * 8.0));
  const L_tang = Math.max(8, radius * 1.0 + Math.min(35, Math.log1p(speed) * 7 + accel * 8.0));

  const axes = [
    { x: Lx * L_grav_pos, y: Ly * L_grav_pos, z: Lz * L_grav_pos, col: colGrav, isCap: true }, // 重力先端キャップあり
    { x: -Lx * L_grav_neg, y: -Ly * L_grav_neg, z: -Lz * L_grav_neg, col: colGrav, isCap: false },
    { x: Nx * L_norm, y: Ny * L_norm, z: Nz * L_norm, col: colNorm, isCap: false },
    { x: -Nx * L_norm, y: -Ny * L_norm, z: -Nz * L_norm, col: colNorm, isCap: false }
  ];

  if (radius >= 2.5) {
    axes.push(
      { x: Mx * L_tang, y: My * L_tang, z: Mz * L_tang, col: colTang, isCap: false },
      { x: -Mx * (L_tang * 0.5), y: -My * (L_tang * 0.5), z: -Mz * (L_tang * 0.5), col: colTang, isCap: false }
    );
  }

  ctx.save();
  // 強大な加速度発生時のみ線を少し太く強調 (1.5px)
  ctx.lineWidth = (accel > 0.8) ? 1.5 : 1.0;
  ...
  for (let i = 0; i < axes.length; i++) {
    ...
    const rp = (typeof rotate3D === "function") ? rotate3D(temp) : temp;
    const pp = (typeof project3D === "function") ? project3D(rp.x, rp.y, rp.z) : null;

    if (pp && pp.visible !== false && !isNaN(pp.x) && !isNaN(pp.y)) {
...
/**
 * 👑【エンジン本体】天体満ち欠け（月相/陰影）描画サブエンジン
 * （※ drawBodyCore の外側に配置してください）
 */
/**
 * 👑【微調整版】ソフトターミネーター ＆ 光量補正付き満ち欠け描画
 */
/**
 * 👑【幾何学完全一致版】天体満ち欠け描画エンジン
 * スクリーン上の太陽位置 (prSunX, prSunY) に対し、光面を厳密に固定
 */
/**
 * 👑【彗星ベクトルモデル】反太陽方向完全固定型 満ち欠け描画エンジン
 * 影の軸を「太陽の正反対方向」に絶対固定し、視角（cosPhi）で影の幅のみを可変制御
 */
/**
 * 👑【偏り完全除去版】楕円パス直接描画による 100% 対称 満ち欠けエンジン
 * ctx.scale を廃止し、ellipse API で右下への行列歪みを代数的に排除
 */
/**
 * 👑【彗星透過投影モデル】3D 空間完全同期型 満ち欠け描画エンジン
 * 彗星の尾と全く同じ 3D ➔ 2D 透視投影パイプラインを通過させ、影軸のズレを 100% 根絶
 */
/**
 * 👑【自立型 Zero-Allocation】3D 空間完全同期型 満ち欠け描画エンジン
 * 外部の変数宣言に依存せず、関数内部で 0 Byte キャッシュを自己保持して ReferenceError を完全防護
 */
function renderBodyPhase(b, sun, cameraPos, pr, prSunX, prSunY, radius) {
  if (radius < 1.2 || !b || !sun || !cameraPos) return;

  // 🛡️ 1. 【完全 Zero-Allocation】初回呼び出し時にのみ 1 回だけキャッシュ用オブジェクトを生成 (0 Byte/frame)
  if (!renderBodyPhase.temp) {
    renderBodyPhase.temp = { x: 0, y: 0, z: 0 };
  }
  const temp = renderBodyPhase.temp;

  // 2. 3D 空間でのベクトル計算 (光源 L, 視線 V)
  const Lx = sun.x - b.x, Ly = sun.y - b.y, Lz = sun.z - b.z;
  const Vx = cameraPos.x - b.x, Vy = cameraPos.y - b.y, Vz = cameraPos.z - b.z;

  const L_len = Math.sqrt(Lx * Lx + Ly * Ly + Lz * Lz);
  const V_len = Math.sqrt(Vx * Vx + Vy * Vy + Vz * Vz);

  if (L_len === 0 || V_len === 0) return;

  // 3. 3D 位相角 cos(φ) (-1.0:新月 〜 0.0:半月 〜 +1.0:満月)
  const cosPhi = (Lx * Vx + Ly * Vy + Lz * Vz) / (L_len * V_len);

  // 4. 👑【彗星アルゴリズムの完全移植】3D 空間上で反太陽方向へ進んだ「影の頂点」を生成
  const nx = -Lx / L_len; // 反太陽方向の 3D 単位ベクトル
  const ny = -Ly / L_len;
  const nz = -Lz / L_len;

  // インプレースプロパティ更新 (メモリ割当 0 Byte)
  temp.x = b.x + nx * 100;
  temp.y = b.y + ny * 100;
  temp.z = b.z + nz * 100;

  // 5. 👑 彗星の尾と全く同じ 3D ➔ 2D 透視投影パイプラインを通す
  const rp = (typeof rotate3D === "function") ? rotate3D(temp) : temp;
  const pp = (typeof project3D === "function") ? project3D(rp.x, rp.y, rp.z) : null;

  // 6. 3D 透視投影された「真の 2D 影角度」の算出
  let shadowAngle = 0;
  if (pp && !isNaN(pp.x) && !isNaN(pp.y) && pp.visible !== false) {
    shadowAngle = Math.atan2(pp.y - pr.y, pp.x - pr.x);
  } else {
    // 安全フォールバック
    shadowAngle = Math.atan2(pr.y - prSunY, pr.x - prSunX);
  }
  ...
  const innerRadiusX = Math.max(0.01, radius * Math.abs(cosPhi));

  if (cosPhi < 0) {
    // 🌒 新月 ➔ 三日月
    ctx.arc(0, 0, radius, -Math.PI / 2, Math.PI / 2, false);
    ctx.ellipse(0, 0, innerRadiusX, radius, 0, Math.PI / 2, -Math.PI / 2, false);
  } else {
    // 🌔 半月 ➔ 満月
    ctx.arc(0, 0, radius, -Math.PI / 2, Math.PI / 2, false);
    ctx.ellipse(0, 0, innerRadiusX, radius, 0, Math.PI / 2, -Math.PI / 2, true);
  }
...
// 👑 ボタン＆キーボード連動の初期化 (Zero-Allocation 仕様)
const btnGizmo = document.getElementById("btnToggleGizmo");

function toggleAxesGizmo() {
  showAxesGizmo = !showAxesGizmo;
  if (btnGizmo) {
    // アクティブ時にボタンの見た目を強調（CSS クラス切替）
    btnGizmo.classList.toggle("active", showAxesGizmo);
  }
}

// ボタンクリックイベント
if (btnGizmo) {
  btnGizmo.addEventListener("click", toggleAxesGizmo);
}

// キーボードショートカット [G] キー連動
window.addEventListener("keydown", (e) => {
  // 入力フォーム等にフォーカスがない時のみ発動
