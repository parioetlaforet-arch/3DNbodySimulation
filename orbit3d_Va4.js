...
let targetHistory = [];
let lvecMode = 0; // 🛑 0:非表示, 1:針のみ, 2:針＋ピザ
...
const DEBUG_PRESETS = {
   ...
   // =====================================================================
    // 🍕 4. ケプラー・ピザ極限観察モード（旧：球状星団高負荷モードからの超進化）
    // =====================================================================
    renderStress: {
        name: "ケプラー・ピザ宇宙数理観察モード（面積速度一定の完全証明）",
        ui: {
            bodyCount: 1,         bodyCountInput: 1,        // 🌌 ターゲットは孤独な1基のみ
            sunMass: 10000,       sunMassInput: 10000,      // ☀️ 主星の絶対的な重力場
            sunVxSlider: 0.0,     sunVxInput: 0.0,
            sunVySlider: 0.0,     sunVyInput: 0.0,
            sunVzSlider: 0.0,     sunVzInput: 0.0,
            objMass: 1.0,         objMassInput: 1.0,
            useFixedObjMass: true,
            gravitySlider: 1.0,   gravityInput: 1.0,
            spawnVelSlider: 0.3,  spawnVelInput: 0.3,       // 🚀 絶妙な楕円を切り取る初期速度
            speedSlider: 1.5,     speedInput: 1.5,          // ⏳ じっくり数理を拝むための滑らかな等倍速
            eps2Input: 0.0,                                 // 💥 軟化補正をゼロにし、純粋なケプラー重力を再現
            cameraRotateSpeed: 0.0005
        },
        physics: () => {
            // 1. システムバッファのパージと天体数の強制制限（1つだけ）
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = 1;
            if (window.bodies) window.bodies.length = 0;
            if (window.targetHistory) window.targetHistory = []; // ピザの履歴を完全クリア

            // 2. 物理コアパラメータの設定
            if (window.settings) {
                settings.sunInitialVx = 0.0; settings.sunInitialVy = 0.0; settings.sunInitialVZ = 0.0;
                settings.gravityMultiplier = 1.0;
                settings.spawnVelocityMultiplier = 0.3; // 楕円軌道化のスパイス
                settings.simSpeed = 1.5;
                settings.eps2 = 0.0; // ニュートン力学100%準拠
                settings.objBaseMass = 1.0;
                settings.useFixedObjMass = true;
                settings.sunFixed = true;
            }

            // 3. 🛠️ 核心：1基の天体を「超ロングな軌跡」かつ「美しき楕円」で宇宙に再誕させる
            // ※もし天体を生成するファクトリー関数（例：spawnBodiesやinitSimulation）が別にある場合は、
            // ここでその初期化関数を呼ぶか、以下のように手動で軌道を直接焼き付ける。
            if (typeof resetSimulation === "function") {
                resetSimulation(); // 一度クリアして1設定で再生成させる
            }

            // ── 🎛️ 4. UIボタンの自動完全同期 ──
            // 不要なUIを消し、軌跡、L-VECボタンをアクティブにする
            const targetON = ['cometTrailBtn', 'planetTrailBtn', 'sunTrailBtn'];
            targetON.forEach(id => {
                const btn = document.getElementById(id);
                if (btn && !btn.classList.contains('toggle-on')) btn.click();
            });

            // 👑 L-VECボタンを強制的に「Mode 2（緑・針＋ピザ）」に書き換える
            window.lvecMode = 2;
            ...
            // 5. カメラを極小スローで自動旋回させ、3Dパースのピザを多角的に鑑賞
            if (typeof syncCameraRotateSpeed === "function") syncCameraRotateSpeed(0.0005, false);
            window.isAutoRotateEnabled = true;
            ...
            // 🛠️【超ロング軌跡ハック】もし軌跡の長さ配列の上限（trailLength等）が可変なら、ここで限界突破させる
            if (window.bodies && window.bodies[0]) {
                const b = window.bodies[0];
                if (b.trail) {
                    // 履歴バッファサイズを拡張（もしシステムが対応していれば500〜1000ターン記憶させる）
                    b.maxTrailPoints = 1000;
                }
            }

            if (window.UI_DEBUG) console.log("🍕 [ケプラー特級呪物] 単体ピザ観察マトリクスが完全執行されました。");
        }
    }
};
...
function updatePhysics(dt) {
  ...
  // 2. 予知フラグを全天体一度フラットに戻す（黒板の消しゴムがけ）
  for (let b of bodies) {
    b.willCollide = false;
    b.timeToCollision = 0;
  }
...
// =====================================================================
    ...
    if (count >= 300) {
      // ① 300個超の過密宇宙：ノイズと描画負荷を抑えるために0.3倍へ引き締め
      densityMultiplier = 0.3;
    } else {
      // ② 299個以下の通常宇宙：デフォルトの倍率
    ...
    // =====================================================================
    // 👑 【あなたの哲学の執行】少ない星を詳細に観察するための絶対規律
    // =====================================================================
   // 観察者心理とクリアな視認性の両立を目指す新しい規律
    if (count <= 15) {
            // 🛸 【修正点 1】惑星は「ちょうど一周」にピキーンと引き締める
      if (b.isOrbiting) {
        // 現在の設定なら「800」が地球の一周分に相当する黄金比。
        // ここを800固定にすることで、何周しても線が太くならず、常に「クリアな一つの輪」に見える。
        limit = 800;
      } else {
        // 🛸 【修正点 2】彗星などの放物線は「無限」のままにする
        // 放物線は一周しないため、これを残すことで「過去の飛来の爪痕」の神秘性は保持する。
        limit = Infinity;
      }
            // この「limit」を基準に、消しゴムを発動させる
      ...
      // 16個以上のときは、画面の窒息を防ぐために通常通り上限でカットする
      while (b.trail.length > limit) b.trail.shift();
    }
}
...
function renderScene() {
 ...
 // ---------------------------------------------------------
  ...
  for (const obj of sortedBodies) {
    ...
    // =========================================================
    // 🎛️ 1番目のスイッチ：【針の制御】（Modeが 1 または 2 のときON）
    // =========================================================
    if (lvecMode > 0 && obj.b !== sun) {
      drawAngularMomentumVectorDirect2D(obj.b, sun, pr);
    }

  } // 👈 👑【超重要！】ここで「天体の個別ループ」の部屋が完全に終わる！！！

  // =========================================================
  // 🎛️ 2番目のスイッチ：【レーダーピザの制御】（Modeが 2 のときのみON）
  // 天体全員の個別描画がすべて終わった「最底面」で、1回だけ単独執行！
  // =========================================================
  if (lvecMode === 2) {
    drawLVecAreaRadar(bodies, sun, targetHistory);
  } else {
    targetHistory = []; // Mode 0 または 1 の時はバッファをクリアしてメモリ解放
  }
...
// ---------------------------------------------------------
// 👑【調律】手前のループで完璧に3D回転が終わっているソート済みの配列を直撃で引き渡す！
 ...
 * 👑【リアル3D空間投影 ＆ メリハリ微調整パラメータ完全分離版】
 */
function drawAngularMomentumVectorDirect2D(b, sun, pr) {
  if (!b || !sun || !pr || isNaN(pr.x) || isNaN(pr.y)) return;

   // =========================================================
  // 🎛️ 【統治者の微調整ツマミ】世界上限6000前提・リミッター最大解放版！
  // =========================================================
  const SENSITIVITY  = 1.5;   // 🚀 2.2から1.5へ少し抑え、巨大な数値を画面内に収める
  const MIN_LENGTH   = 0;     // 📉 最小時は極限まで縮み、静寂を表現
  const MAX_LENGTH   = 1000;  // 👑 350から1000へ限界突破！画面を突き抜ける巨塔を許可

  // 1. 太陽中心の位置ベクトル r (相対位置) ＆ 速度ベクトル v (相対速度)
  const rx = b.x - sun.x;
  const ry = b.y - sun.y;
  const rz = b.z - sun.z;
  ...
  if (r_len > 0) {
    ...
    const ey = (v2 * ry - r_dot_v * vy) / mu - ry / r_len; // タイポ修復済の規律を維持
    const ez = (v2 * rz - r_dot_v * vz) / mu - rz / r_len; // タイポ修復済の規律を維持
    ecc = Math.sqrt(ex*ex + ey*ey + ez*ez);
  }

  // 決定カラーマッピング
  let eccColor = "#00ff88";
  if (ecc < 0.05)       eccColor = "#00bcff"; // 🟦 完全調和
  else if (ecc < 0.20)  eccColor = "#00ff88"; // 🟩 標準楕円
  else if (ecc < 0.60)  eccColor = "#ffff00"; // 🟨 乱調楕円
  else if (ecc < 1.00)  eccColor = "#ff9900"; // 🟧 彗星軌道
  else                  eccColor = "#ff0044"; // 🟥 無限追放
...
function drawGravityCenterOfTop2(passedSortedBodies) {
  // 🌟【復権】表示フラグ（トグル）がOFFなら、何があろうと即座に完全撤退する
  if (!window.showBarycenter) return;

  // 👑 既に3D回転済みのソート配列が渡されていない場合は安全に離脱
  if (!passedSortedBodies || passedSortedBodies.length === 0) return;

  // 1. 渡された3D回転済みの天体たちを、まず質量の降順でソートしてガッチリ確定！
  const sortedNodes = [...passedSortedBodies]
    .filter(item => item && item.b)
    .sort((a, b) => {
      const massA = parseFloat(a.b.mass !== undefined ? a.b.mass : (a.b.m !== undefined ? a.b.m : 0));
      const massB = parseFloat(b.b.mass !== undefined ? b.b.mass : (b.b.m !== undefined ? b.b.m : 0));
      return massB - massA;
    });

  // 2. ソート済みの配列から、質量を持った生存天体を安全に抽出
  const allValidNodes = sortedNodes.filter(n => {
    const m = parseFloat(n.b.mass !== undefined ? n.b.mass : (n.b.m !== undefined ? n.b.m : 0));
    return m > 0 && !isNaN(m);
  });

  // =====================================================================
  // 👑 【スマートアジャスト】トグルの設定値（Stage）に100%絶対服従する規律
  // =====================================================================
  // あなたのトグル（Stage）が「全体」を指した時、window.barycenterTargetCount には
  // おそらく 999 などの大きな数、または 3000（全天体数）、あるいは文字列が代入されます。
 // =====================================================================
  // 👑 【完全包囲】トグルの裏の数値を100%制圧する最終調律
  // =====================================================================
  const targetCount = parseFloat(window.barycenterTargetCount);
    // 🌟【新・絶対規律】
  // カウントが「1, 2, 3, 4」の時だけを明確に少数モード（isAllMode = false）と認定する！
  // それ以外（4より大きい、0以下、NaN、文字列など）が来たら、すべて「全体モード」と見なす。
  const isAllMode = (targetCount > 4 || targetCount <= 0 || isNaN(targetCount));
    // 計算対象の数を確定
...
// ── 3. 【核心の規律】どんなオブジェクト構造が来ても絶対に死なない座標抽出 ──
  for (let i = 0; i < n; i++) {
    const node = allValidNodes[i];
    if (!node) continue;
        const m = parseFloat(node.b.mass !== undefined ? node.b.mass : (node.b.m !== undefined ? node.b.m : 0));
    if (m <= 0 || isNaN(m)) continue;

    // 🌟【超強力アジャスト】.r.x があれば使い、無ければ直下の .rx や .x、あるいは body の中を探す
    let rx = 0, ry = 0, rz = 0;
        if (node.r && typeof node.r.x === 'number' && !isNaN(node.r.x)) {
      rx = node.r.x; ry = node.r.y; rz = node.r.z;
    } else if (typeof node.rx === 'number' && !isNaN(node.rx)) {
      rx = node.rx; ry = node.ry; rz = node.rz;
    } else if (typeof node.x === 'number' && !isNaN(node.x)) {
      rx = node.x; ry = node.y; rz = node.z;
    } else if (node.b && typeof node.b.x === 'number' && !isNaN(node.b.x)) {
      rx = node.b.x; ry = node.b.y; rz = node.b.z;
    } else {
      continue; // 座標がどうしても見つからないチリは安全にスキップ
    }
  ...
  // ── 4. 回転済みの重心座標を2Dスクリーンへ透視投影 ──
  const ppBary = project3D(rotatedBaryX, rotatedBaryY, rotatedBaryZ);
    // 🌟【デバッグ通信】もし画面に表示されない場合、コンソール（F12）に何が原因か一撃で吐き出すパッチ
  if (isAllMode) {
    console.log(`[全体重心デバッグ] 算入数(n):${n}, 全質量:${totalMass.toFixed(1)}, 画面X:${ppBary?.x}, 画面Y:${ppBary?.y}, 視認性:${ppBary?.visible}`);
  }

  if (!ppBary || isNaN(ppBary.x) || isNaN(ppBary.y)) return;

  // =====================================================================
  // 🔮 【アジャスト：宇宙の純粋パースペクティブへの完全同期】
  // =====================================================================
  ...
  // ── 5. 各天体ノードへ放射状に伸びる「引力のドットライン」 ──
  // 全体モード（isAllMode）の時はノイズ遮断のため点線を完全スキップ！
  if (!isAllMode) {
    ...
    ctx.setLineDash([3 * zScale, 3 * zScale]);
      ...
      if (ppNode && !isNaN(ppNode.x) && !isNaN(ppNode.y)) {
        ctx.beginPath();
        ctx.moveTo(ppBary.x, ppBary.y);
        ctx.lineTo(ppNode.x, ppNode.y);
        ctx.stroke();
      }
    }
    ctx.setLineDash([]);
 ...
 // ── 6. 精密照準（レティクル）の強制スタンプ ──
  // 🌟【個数限定なし・全体モード特権アジャスト】
  // 全天体モードの時は、どれだけ遠くても、光の中に隠れても絶対に視認できるよう、
  // サイズを通常の3倍（48px）、線の太さも極太（4px）に強制覚醒させて存在感を主張する！
  const baseSize = isAllMode ? 48 : 16;
  const minSize  = isAllMode ? 24 : 4;
  const size     = Math.max(minSize, baseSize * zScale);
    ctx.strokeStyle = isAllMode ? "#ff00ff" : "#ffff00"; // ★全体モードの時はマゼンタピンク（紫色）に変えると、星の黄色や白の光を完全にぶち抜けます！
  ...
  // コアサークル（全体の時は少し大きめの輪にする）
  ctx.lineWidth = isAllMode ? 2 : Math.max(1, 2 * zScale);
  ctx.beginPath();
  ctx.arc(ppBary.x, ppBary.y, isAllMode ? 14 : Math.max(2, 7 * zScale), 0, Math.PI * 2);
  ctx.stroke();

  // ── 7. HUDデータ計器盤（DOM）へのテキスト射出 ──
  ...
  if (elName) {
    if (isAllMode) {
      elName.textContent = `Barycenter (All-Body System Total): ${n} Active Stars Locked`;
    } else {
      const names = targetNodes.map(n => n.body.name || "Unknown").join(" + ");
      elName.textContent = `Barycenter (${n}-Body): ${names}`;
    }
  }
  if (elPos) {
    elPos.textContent = `Barycenter Pos: X:${ppBary.x.toFixed(1)} Y:${ppBary.y.toFixed(1)} Z:${rotatedBaryZ.toFixed(1)}`;
  }
  if (elMass && !isAllMode) {
    const massStrings = targetNodes.map(n => `${n.body.name || "Star"}:${n.mass.toFixed(1)}`);
    elMass.textContent = `Mass Matrix -> ` + massStrings.join(" / ");
  } else if (elMass && isAllMode) {
    elMass.textContent = `Total Net Mass Matrix -> System Total: ${totalMass.toFixed(1)}`;
  }
...
window.executeButtonCoreLogic = function(id, element) {
    ...
    switch (id) {
        case "toggleBaryBtn":
            ...
            // 5. ボタン表面の文字を、現在のステージと言語に合わせて動的変調
            if (currentStage === 0) {
...
/**
 * Sun以外の最大質量天体を自動ロックし、一定ターンの「面積速度（ピザ）」を
 * 3D投影して【ネオン放射グラデーション】のレーダー残像として描画するギミック
 */
function drawLVecAreaRadar(bodies, sun, targetHistory) {
  ...
  if (targetHistory.length > 300) {
    targetHistory.shift();
  }

  if (targetHistory.length < 10) return;
  ...
  for (let i = 1; i < targetHistory.length; i++) {
    ...
    // ── 🛠️ 2D画面上での太陽からターゲットまでの距離を算出 ──
    const dx = pCurr.x - pSun.x;
    const dy = pCurr.y - pSun.y;
    const distance2D = Math.sqrt(dx * dx + dy * dy);

  // ── 🎨 核心：太陽中心からの放射グラデーションの生成（高コントラスト版） ──
    const grad = ctx.createRadialGradient(
      pSun.x, pSun.y, 0,           // 内円（太陽の中心点）
      pSun.x, pSun.y, distance2D   // 外円（天体の現在地までの半径）
    );

    // 過去のターンほど「急激に」薄くするために、ageFactorを非線形（2乗）に強化
    // これにより最新のピザが圧倒的に濃く、過去の残像がグッと淡くなる
    const ageFactor = Math.pow(i / targetHistory.length, 1);
        // 【濃淡の規律】
    // 0.0〜0.15までの超至近距離（太陽の根元）に圧倒的な光量を集中させる
    grad.addColorStop(0,    `rgba(0, 255, 136, ${0.45 * ageFactor})`); // 💥 芯はほぼ不透明に近い激濃ネオン
    grad.addColorStop(0.15, `rgba(0, 255, 136, ${0.18 * ageFactor})`); // 📉 ここから急激に減衰開始
    grad.addColorStop(0.4,  `rgba(0, 255, 136, ${0.08 * ageFactor})`); // 👤 中間はすでにうっすら透ける影
    grad.addColorStop(1,    `rgba(0, 255, 136, 0.0)`);                  // 🌌 天体到達時は完全に闇へ溶ける

    // ── レンダリング執行 ──
    ctx.fillStyle = grad;
    // 外枠の線もグラデーションの濃淡に合わせて、より繊細に浮き立たせる
    ctx.strokeStyle = `rgba(0, 255, 136, ${0.0 * ageFactor})`;
    ctx.lineWidth = 0.0;
    ...
    ctx.closePath();
        ctx.fill();   // 濃厚なコントラストで塗りつぶし
    ctx.stroke();
  }

  // ロックオンHUDテキスト（既存のテキストスタイルへ完全同期）
  const lockRot = rotate3D({ x: target.x, y: target.y, z: target.z });
  const pLock = project3D(lockRot.x, lockRot.y, lockRot.z);
  if (pLock.visible) {
    // 🎨 色、フォント、アライメント、ベースラインを完全にコピー
    ctx.fillStyle = "rgba(0, 255, 136, 0.6)"; // レーダーの色（またはお好みで target.color など）
    ctx.font = "9px monospace";
    ctx.textAlign = "left";
    ctx.textBaseline = "middle";
        // 表示テキストと座標のオフセット（+4, -2）を完全に同期
    const displayName = target.name || "TRGT";
    ctx.fillText(`${displayName} [A-VEC]`, pLock.x + 4, pLock.y - 2);
  }
