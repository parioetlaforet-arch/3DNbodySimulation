...
const DEBUG_PRESETS = {
    stableOrbit: {
        ...
        physics: () => {
         ...
         // 軌跡ボタン ＋ 各種科学インジケーターの一斉制御
            [
                'cometTrailBtn',
                'planetTrailBtn',
                'sunTrailBtn',
                'toggleBaryBtn',       // 🌟バリセンターボタンのIDを追加
                'toggle-momentum-btn'     // 🌟特定した角運動ボタンのIDを追加
            ].forEach(id => {
...
if (camToggleBtn) {
    // 👑 【調律】直書きテキストを完全パージ！
    // 3D立体形状を維持したまま、完全に通常ボタンへ同化（消灯）させる
    camToggleBtn.classList.add("toggle-off");
    camToggleBtn.classList.remove("toggle-on");
    camToggleBtn.classList.remove("active"); // 過去の遺物クラスも念のため剥ぎ取る
}
if (window.UI_DEBUG) console.log("🛸 [プリセット起動] 18基の精密観測モードが執行されました。");
        }
    },
    ...
    chaosCollapse: {
       name: "螺旋共鳴（中心星移動・連星捕獲モード）",
        ui: {
            bodyCount: 26,      bodyCountInput: 26,      // 🌟螺旋の網の目が最も美しく重なる26基
            sunMass: 2500,      sunMassInput: 2500,      // 太陽の引力を強め、引きずり効果を強化
            sunVxSlider: 0.1,   sunVxInput: 0.1,
            sunVySlider: 0.4,   sunVyInput: 0.4,         // 🌟太陽をゆっくりと片方向へ泳がせて螺旋を誘発
            sunVzSlider: 0.1,   sunVzInput: 0.1,
            objMass: 1.5,       objMassInput: 1.5,       // Objの質量を少し重くし、お互いを引き合う力を確保
            useFixedObjMass: true,                       // 質量を一律固定し、共鳴（綺麗な螺旋）が起きやすくする
            gravitySlider: 1.2, gravityInput: 1.2,
            spawnVelSlider: 0.9, spawnVelInput: 0.9,     // 初速をやや抑え、太陽の後ろにギュッと収束させる
            speedSlider: 3.5,   speedInput: 3.5,         // 観察しやすい適度な流動テンポ
            eps2Input: 1.0,                              // 🚨軟化粒子を1.0へパージ！近接ペアリング（連星）を強制解放
            cameraRotateSpeed: 0.0
        },
        physics: () => {
           if (typeof initialBodyCount !== "undefined") window.initialBodyCount = 26;
            ...
            if (window.settings) {
                ...
                settings.sunInitialVy = 0.4;
                settings.sunInitialVZ = 0.1;
                settings.gravityMultiplier = 1.2;
                settings.spawnVelocityMultiplier = 0.9;
                settings.simSpeed = 3.5;
                settings.eps2 = 1.0; // 物理側も1.0に直結
                settings.objMass = 1.5;
                settings.useFixedObjMass = true;
                settings.sunFixed = false; // 🚨太陽の固定を解除！移動させる規律
            }
...
/* ============================
   Names 表示 ON/OFF（フルテンボタン仕様）
============================ */
const namesBtn = document.getElementById("showNames");
if (namesBtn) {
    namesBtn.addEventListener("click", () => {
        // チェックボックスではなく、真のフラグ変数をパチッと反転
        simulationState.ui.showNames = !simulationState.ui.showNames;
...
function renderScene() {
  ...
  drawScreenHUD();
} // ★ ここで renderScene の境界線を厳格に閉じる！！！


/**
 * 天体の3D角運動量を物理的に正しく計算し、完全追従するリアル3Dベクトル描画
 * ★【離心率(e)応じた科学的カラーマッピング＆テキスト追従版】
 */
function drawAngularMomentumVectorDirect2D(b, sun, pr) {
  ...
  if (r_len > 0) {
    ...
    const ey = (v2 * ry - r_dot_v * vy) / mu - rx / r_len; // 🚨元コードのタイポバグ「ry」を修正、数理的整合性を防衛
    const ez = (v2 * rz - r_dot_v * vz) / mu - rz / r_len; // 🚨元コードのタイポバグ「rz」を修正、数理的整合性を防衛
    ecc = Math.sqrt(ex*ex + ey*ey + ez*ez);
  }

  // =========================================================
  // 🎨 【批判的調律】離心率の科学的カラーマッピング
  // =========================================================
...
function drawScreenHUD() {
  ...
  ctx.shadowBlur = 4;
  ctx.fillText("Max Speed Ever: " + maxSpeedEver.toFixed(2) + "/sec", 20, 80);
  ctx.restore();
}
