...
function drawAngularMomentumVectorDirect2D(b, sun, pr) {
  ...
  // =========================================================
  // 🎛️ 【統治者の微調整ツマミ】ここをイジってメリハリを支配せよ！
  // =========================================================
  const SENSITIVITY  = 2.2;   // 🚀 針の伸びの勢い（倍率）。大きくするとより長く伸びる
  const MIN_LENGTH   = 1;     // 📉 最低保証の長さ(px)。1や0にすると短くなる方向へ極限まで縮む
  const MAX_LENGTH   = 350;   // 限界突破の最大長さ(px)。画面を贅沢に使うなら350〜400
  ...
  if (r_len > 0) {
    ...
    const ey = (v2 * ry - r_dot_v * vy) / mu - rx / r_len; // タイポ修復済の規律を維持
