...
const DEBUG_PRESETS = {
    ...
    preset1: {
        ...
        ui: {
            bodyCount: 199,          bodyCountInput: 199,   // 🎯 199基の超過密・リアルタイムN-body群
            sunMass: 3000,          sunMassInput: 3000,   // 重心運動を際立たせる超大質量主星
            sunVxSlider: 0.0,      sunVxInput: 0.0,
            sunVySlider: 0.0,      sunVyInput: 0.0,
            sunVzSlider: 0.0,      sunVzInput: 0.0,
            objMass: 1.0,          objMassInput: 1.0,
            useFixedObjMass: false,
            gravitySlider: 1.2,    gravityInput: 1.2,    // 天体同士の引き合い（相互作用）が美しく出る重力倍率
            spawnVelSlider: 1.0,   spawnVelInput: 1.0,    // 楕円軌道と重力スイングバイを誘発する標準初速
            speedSlider: 1.5,      speedInput: 1.5,      // 軌道描画線の交差（ワイヤーフレーム）を熟視できるクロック速度
            eps2Input: 30,                                // 滑らかな近接散乱を保証する軟化パラメータ
            cameraRotateSpeed: 0.0008                     // 3D立体感を際立たせる緩やかな自動カメラ旋回
        },
        physics: () => {
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = 199;
            if (window.bodies) window.bodies.length = 199;
            ...
            // 🎯 『L-Needle（角運動・離心率針）』を最初からONにして199個のベクトル場を全開表示！
            const targetON  = ['planetTrailBtn', 'cometTrailBtn', 'toggleBaryBtn', 'toggle-momentum-btn'];
            const targetOFF = ['sunTrailBtn'];
            ...
            if (typeof syncCameraRotateSpeed === "function") syncCameraRotateSpeed(0.0008, true);
            ...
            if (window.UI_DEBUG) console.log("💥 [PRST: 1] 199天体 N-body 重力カオス・スペクタクルモードが起動しました。");
        }
    },
    // 🪐 2. 双太陽・二重連星系（「連」の宇宙）
    preset2: {
...
function renderScene() {
  ...
  for (const obj of sortedBodies) {
    ...
    // =========================================================
    // 🌌 最終段階：ドット ＋ 針 ＋ 色 のみの極小レンダリング（小天体専用）
    // =========================================================
    // 💡 太陽（isSunObj）以外の場合のみ、極小レンダリングを適用する！
    if (isMinimalMode && !isSunObj) {
      // 1. 純粋なドット（小天体位置）を極小サイズ(1.5px)で描画
