...
document.addEventListener("DOMContentLoaded", () => {
 ...
 // 2. ★【大改造開通】巡回式マルチ・バリセンターボタンの配線
    ...
    if (baryBtn) {
        // 👑 アロー関数（=>）ではなく通常の function() を使うことで「this（ボタン本体）」を固定
        baryBtn.addEventListener("click", function() {
            // 余計な古いON/OFFロジックをすべてパージ（消去）し、中央管制関数へ直結！
...
// =====================================================================
    ...
    // =====================================================================
    if (count <= 15) {
      // 🪐 天体数が15個以下のときは、消しゴム（shift）を完全に「完全免除」する！
      // 過去の座標を1ミリも捨てず、星が歩んだ「すべての歴史（情報）」を画面に焼き付け続ける。
    } else {
...
/**
 * 質量上位N個（任意の複数個体）の天体間における合成重心（マルチ・バリセンター）を計算・描画・出力する
 * ★【九龍城・立体座標完全同期版】
 */
function drawGravityCenterOfTop2(passedSortedBodies) {
  ...
  // 1. 渡された3D回転済みの天体たちを、さらに質量の降順でソートして抽出
  ...
  // 2. 要求された計算数（2〜4体）と生存数で安全クランプ
  const n = Math.min(window.barycenterTargetCount, sortedNodes.length);
  if (n < 1) return;
  ...
  // 3. 【核心の規律】すでに完璧に計算されている「回転後の3D空間座標（r.x, r.y, r.z）」から直接重心を合成！
  for (let i = 0; i < n; i++) {
    const node = sortedNodes[i];
    const m = parseFloat(node.b.mass !== undefined ? node.b.mass : (node.b.m !== undefined ? node.b.m : 0));
    if (m <= 0 || isNaN(m)) continue;

    // 生座標ではなく、カメラ回転がかかった後の3D座標を直接ブレンドする
    sumMx += m * node.r.x;
    sumMy += m * node.r.y;
    sumMz += m * node.r.z;
    totalMass += m;
    targetNodes.push({ body: node.b, rx: node.r.x, ry: node.r.y, rz: node.r.z, mass: m });
  }
  ...
  // 👑【グラフィックス聖域化】他の関数のアルファ値や線の太さの汚染を完全遮断
  ctx.save();
  ctx.globalAlpha = 1.0;

  // 5. 各天体ノードへ放射状に伸びる「引力のドットライン」をレンダリング
  ...
  ctx.setLineDash([]); // ドット線の解除

  // 6. 精密照準（レティクル）の強制スタンプ（警告発光イエロー）
  const size = 16;
  ctx.strokeStyle = "#ffff00"; // 鮮烈イエロー
  ctx.lineWidth = 3;            // 極太3px仕様
  ...
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.arc(ppBary.x, ppBary.y, 7, 0, Math.PI * 2);
  ctx.stroke();
  ...
  if (elPos) {
    elPos.textContent = `Barycenter Pos: X:${ppBary.x.toFixed(1)} Y:${ppBary.y.toFixed(1)}`;
  }
  if (elMass) {
 ...
 // ========================================================
    ...
    break;
        ...
        case "toggleBaryBtn":
            ...
            // ① 現在のステージを取得（0: OFF, 2: 2体, 3: 3体, 4: 4体）
            let currentStage = parseInt(targetElement.getAttribute("data-bary-stage")) || 0;

            // ② ステージのローテーション（0 ➔ 2 ➔ 3 ➔ 4 ➔ 0）
            if (currentStage === 0) {
                currentStage = 2;
            ...
            // ③ 属性の更新
            targetElement.setAttribute("data-bary-stage", currentStage.toString());

            // ④ 画面のボタンテキストを言語環境に合わせて動的強制変調（アライメント）
            const isJapanese = (document.getElementById("toggleLangBtn")?.innerText.includes("EN") === false);
            // ※「Language: EN」と表示されている時は日本語モード（またはその逆の既存仕様に100%合わせて調整してください）
                        if (currentStage === 0) {
                targetElement.innerText = isJapanese ? "重心: 全閉" : "BARY: OFF";
            } else {
                targetElement.innerText = isJapanese ? `重心: ${currentStage}天体` : `BARY: ${currentStage}-BODY`;
            }
