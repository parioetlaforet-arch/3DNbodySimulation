...
preset4: {
    name: "PRST: 4 (純粋N体)",
    ui: {
            bodyCount: 100,         bodyCountInput: 100,   // 連星軌道が見やすい適正数量
            sunMass: 2500,        sunMassInput: 2500,   // 主星
            sunVxSlider: 0.0,     sunVxInput: 0.0,
            sunVySlider: 0.0,     sunVyInput: 0.0,
            sunVzSlider: 0.0,     sunVzInput: 0.0,
...
// 2. ★【大改造開通】巡回式マルチ・バリセンターボタンの配線
  ...
  if (baryBtn) {
      ...
      console.warn(" [配線不発] HTML側に id='toggleBaryBtn' のボタンが見つからない！");
  }
...
/* ========================================================
   宇宙管制盤：軌跡レンダリング独立制御マトリクス（クラス同期補正版）
======================================================== */

// 1. 太陽軌跡ボタン（SUN: P）
sunTrailBtn.onclick = () => {
  const isShow = !simulationState.ui.showSunTrail;
    // 状態の二重同期（両方のオブジェクトを確実に更新）
  simulationState.ui.showSunTrail = isShow;
  if (typeof settings !== "undefined") {
    settings.showSunTrail = isShow;
  }

  // ★ クラスの一括・完全制御（on / off / active を同期）
  sunTrailBtn.classList.toggle("active", isShow);
  sunTrailBtn.classList.toggle("toggle-on", isShow);
  sunTrailBtn.classList.toggle("toggle-off", !isShow);
};
...
planetTrailBtn.onclick = () => {
  ...
  if (typeof settings !== "undefined") {
    settings.showPlanetTrail = isShow;
  }
...
cometTrailBtn.onclick = () => {
  // settings または simulationState のどちらからでも安全に反転
  const currentVal = (typeof settings !== "undefined" && settings.showCometTrail !== undefined)
    ? settings.showCometTrail
    : (simulationState.ui.showCometTrail || false);
      const isShow = !currentVal;

  // 状態の二重同期
  if (typeof settings !== "undefined") settings.showCometTrail = isShow;
  if (simulationState.ui) simulationState.ui.showCometTrail = isShow;

  // ★ クラスの一括・完全制御（ここが漏れていたため点灯しっぱなしになっていました）
  cometTrailBtn.classList.toggle("active", isShow);
  cometTrailBtn.classList.toggle("toggle-on", isShow);
  cometTrailBtn.classList.toggle("toggle-off", !isShow);
};
...
/* ========================================================
   軌跡3兄弟 ON/OFF（全方位プロパティ表記ブレ完全吸収版）
======================================================== */

// 1. 彗星軌跡ボタン (CMT)
cometTrailBtn.onclick = () => {
  // 現在の状態を取得（単数形・複数形のどちらからでも拾う）
  ...
  // 💡 考えられるすべてのフラグに『next』を一斉書き込み！
  simulationState.ui.showCometTrail = next;
  simulationState.ui.showCometsTrail = next; // 複数形ケア
  if (typeof settings !== 'undefined') {
...
planetTrailBtn.onclick = () => {
  ...
  simulationState.ui.showPlanetsTrail = next; // 複数形ケア
  if (typeof settings !== 'undefined') {
...
function drawAngularMomentumVectorDirect2D(b, sun, pr, hideText = false) {
 ...
 // 💡 離心率(e)のリアルタイム計算（幾何学優先 ＆ 物理定数同期版）
  ...
  if (b.periPoint && b.aphoPoint && b.periPoint.rMag && b.aphoPoint.rMag) {
    ...
    // ② 未固定時は、物理シミュレーター全体の正当な G と sun.mass から正しくベクトル計算
    const r_len = Math.sqrt(rx * rx + ry * ry + rz * rz);
    if (r_len > 0) {
      const v2 = vx * vx + vy * vy + vz * vz;
      const r_dot_v = rx * vx + ry * vy + rz * vz;
            // シミュレーター本体の標準値 (G: 0.5, sun.mass: 2000.0) に同期
      const realG = (typeof G !== "undefined") ? G : 0.5;
      const realSunMass = (sun && sun.mass) ? sun.mass : 2000.0;
      const mu = realG * realSunMass;
...
// ── 🛡️【完全アライメント】body.x ではなく、構造体の実体である node.b.x へ厳格に修正 ──
 ...
 // 4. 精密照準レティクルのスタンプ（ふた回り縮小・精密HUD仕様）
  // 【変更前】 const baseSize = isAllMode ? 48 : 16;  minSize = isAllMode ? 24 : 4;
  const baseSize = isAllMode ? 24 : 8;  // ★ ベースサイズを半分に縮小
  const minSize  = isAllMode ? 12 : 2;  // ★ 最小サイズも縮小

  const size = Math.max(minSize, baseSize * zScale);
  ctx.strokeStyle = isAllMode ? "#ff00ff" : "#ffff00";
  ctx.lineWidth = isAllMode ? 2 : Math.max(1, 1.5 * zScale); // 線の太さも少し細く補正
  ...
  ctx.beginPath();
  ctx.arc(ppBary.x, ppBary.y, isAllMode ? 8 : Math.max(1.5, 4 * zScale), 0, Math.PI * 2); // ★ 半径も14/7から8/4へ縮小
  ctx.stroke();
...
// ── 6. ★【L-VEC(V) 連動】個別バリセンター（Sun ↔ Target）の自動追跡描画 ──
  ...
  if (isLVecActive && allValidNodes.length >= 2) {
    const sunNode = allValidNodes[0]; // 最も重い主星（Sun）
    const targetNodeItem = getSelectedTargetBody(allValidNodes); // LOCK指定またはAUTOのターゲット天体

    if (sunNode && targetNodeItem && sunNode.item !== targetNodeItem) {
      // 質量と座標の安全取り出し
      const m1 = sunNode.m;
      const m2 = parseFloat(targetNodeItem.b ? (targetNodeItem.b.mass || targetNodeItem.b.m || 0) : (targetNodeItem.m || 0));
      ...
      if (m1 > 0 && m2 > 0) {
        ...
        // 3D透視投影
        const ppLocalBary = project3D(localBaryX, localBaryY, localBaryZ);

        if (ppLocalBary && !isNaN(ppLocalBary.x) && !isNaN(ppLocalBary.y)) {
          const localScale = ppLocalBary.scaleFactor * 0.5;
                    ctx.save();
          // シアン（水色）の戦術カラー
         ...
         // 個別バリセンター照準（コンパクト仕様）
  // 【変更前】 const crossSize = Math.max(5, 12 * localScale);
  const crossSize = Math.max(3, 6 * localScale); // ★ 十字の長さを半分程度に縮小
  ...
  // 円のサイズも縮小
  ctx.beginPath();
  ctx.arc(ppLocalBary.x, ppLocalBary.y, Math.max(1.5, 3 * localScale), 0, Math.PI * 2); // ★ 半径5から3へ縮小
  ctx.stroke();

          // 2. Sun ↔ ターゲット天体を結ぶ薄い水色破線ガイド
          const ppSun = project3D(p1.x, p1.y, p1.z);
          const ppTarget = project3D(p2.x, p2.y, p2.z);
          ...
          // 3. 照準横のスマートラベル
          const targetName = (targetNodeItem.b && targetNodeItem.b.name) ? targetNodeItem.b.name : "Obj";
