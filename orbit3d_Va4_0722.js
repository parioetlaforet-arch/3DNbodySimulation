...
// =========================================================
// 2. settings（GitHubPages公開・ストイック目利き特化版）
// =========================================================
const settings = {
  gravityMultiplier: 1.0,
    // ⚡ 速度の調律：4.0から2.5へ一時ホールド。
  // 起動直後の「白一色の幾何学」をユーザーが目で追える、最も知的な速度感。
  simSpeed: 2.5,
    // 💫 0.95を維持。完全な円ではなく、最初から「目利き」のしがいがある綺麗な楕円を生む黄金比。
  spawnVelocityMultiplier: 0.95,
    // 🎨 軌跡の長さ：1.2をホールド（約2秒強の白い歴史を画面に美しく残す）。
  trailLengthMultiplier: 1.2,
    // 🛑【修正】古い距離別設定をパージし、私たちの「離心率バインド（eccentricity）」へ思想を統一。
  trailColorMode: "eccentricity",
    fullGravityThreshold: 200,
  eps2: 36,                      // 鋭いスイングバイと安全性の物理的均衡点

  objMass: 1.0,
  useFixedObjMass: false,
    // 🪐 スポーン領域のリバランシング（天体数はおよそ24〜32基を想定）
  spawnSettings: {
    minMass: 10.0,
    maxMass: 20.0,    // 質量の最大値を上げ、合体時のサイズ変化（成長）を強調
        // 💡【修正】近すぎると即死カオスになるため、50 ➔ 120 へ引き下げ。
    // 中心に「連星のダンス」を見せる美しい空間（余白）を確保し、軌道の形をクッキリ見せる。
    minDist: 120,
    maxDist: 800
  }
};
...
const DEBUG_PRESETS = {
    ...
    preset1: {
        name: "PRST: 1 (安定軌道・基準宇宙)",
        ui: {
            // 💡【修正】起動時の規律に合わせ、数量を「24」へ完全バインド
            bodyCount: 24,      bodyCountInput: 24,
            ...
            eps2Input: 15,
            cameraRotateSpeed: 0.0001
        },
        physics: () => {
            // 💡【修正】内部の配列制限バッファも「24」へアライメント
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = 24;
            if (window.bodies) window.bodies.length = 24;
            ...
            // 💡【修正】'toggle-momentum-btn' (L-VEC) を ON リストからパージ！
            // PRST:1 を押した瞬間は、まず「純白の美しい円軌道」を見せてユーザーの目を試します。
            const targetON  = ['planetTrailBtn'];
const targetOFF = ['toggle-momentum-btn', 'toggleBaryBtn', 'cometTrailBtn', 'sunTrailBtn'];
...
targetOFF.forEach(id => {
    ...
    if (btn) {
        ...
        btn.classList.remove('toggle-on', 'active');
            ...
            if (window.UI_DEBUG) console.log("🛸 [PRST: 1] 24基の精密観測・自動微回転モードが執行されました。");
        }
    },

    // 🌀 2. 螺旋共鳴（混沌・連星捕獲）
    preset2: {
        name: "PRST: 2 (螺旋共鳴・カオス崩壊)",
        ui: {
        ...
        physics: () => {
            ...
            // 💡【修正】ここからも L-VEC をパージ。ぐちゃぐちゃに絡み合う「白髪の嵐」を自力観察させます
           const targetON  = ['cometTrailBtn', 'planetTrailBtn', 'sunTrailBtn'];
const targetOFF = ['toggleBaryBtn', 'toggle-momentum-btn'];
...
targetOFF.forEach(id => {
    ...
    if (btn) {
        ...
        btn.classList.remove('toggle-on', 'active');
            ...
            if (window.UI_DEBUG) console.log("🛸 [PRST: 2] 26基の螺旋共鳴カオスモードが執行されました。");
        }
    },

  preset3: {
        name: "PRST: 3 (潮汐破壊・高速スイングバイ)",
        ui: {
            ...
            useFixedObjMass: false, // 質量をばらつかせるため一時的にOFFに同期
        ...
        physics: () => {
            const numBodies = 200; // 最適密度の200基
            if (typeof initialBodyCount !== "undefined") window.initialBodyCount = numBodies;
            ...
            if (window.settings) {
                ...
                settings.useFixedObjMass = false; // 質量変動プロトコルを有効化
                settings.sunFixed = false;
            }

            // =======================================================
            // 🌀 【潮汐破壊専用】べき乗質量・不均等距離配置の動的再生成
            // =======================================================
            if (window.bodies) {
                window.bodies = []; // 一旦バッファをクリア

                // 1. 太陽（巨大重力源）をインデックス0に配置
                bodies.push({
                    x: -1200, y: 300, z: 0, // 高速双曲線軌道で進入する初期位置
                    vx: 3.5, vy: -0.5, vz: 0,
                    mass: 3000,
                    size: 25,
                    color: "#ff3300",
                ...
                for (let i = 1; i < numBodies; i++) {
                    // 【距離の不均等配置】対数的なすり鉢配置（300〜2500pxのレンジ）
                    const randVal = Math.random();
                    const r = 300 + Math.pow(randVal, 2.5) * 2200;
                                        const theta = Math.random() * Math.PI * 2;
                    const phi = Math.acos((Math.random() * 2) - 1); // 3次元全天球散布

                    const x = r * Math.sin(phi) * Math.cos(theta);
                    const y = r * Math.sin(phi) * Math.sin(theta);
                    const z = r * Math.cos(phi) * 0.3; // 縦方向に少し潰したパンケーキ状

                    // 【質量の極端なべき乗分布】
                    let mass = 0.01;
                    const massRoll = Math.random();
                    if (massRoll > 0.98) {
                        // 2%の確率で「巨大ガス惑星級」を生成
                        mass = 50.0 + Math.random() * 70.0;
                    } else if (massRoll > 0.85) {
                        // 13%の確率で「中型岩石惑星級」を生成
                        mass = 2.0 + Math.random() * 10.0;
                    } else {
                        // 残り85%は「微細なデブリ・小天体級」
                        mass = 0.005 + Math.random() * 0.2;
                    }

                    // 質量に応じたサイズ決定
                    const size = mass > 10 ? Math.pow(mass, 0.3) * 1.5 : 0.5 + mass * 2;

                    // 速度は太陽の重力井戸に緩やかに捕らえられている初期公転速度（公転方向を揃える）
                    const dist = Math.sqrt(x*x + y*y + z*z) || 1;
                    const vBase = Math.sqrt(1.5 * bodies[0].mass / dist) * 0.3; // やや遅い初期公転
                    const vx = (-y / dist) * vBase + (Math.random() - 0.5) * 0.2;
                    const vy = (x / dist) * vBase + (Math.random() - 0.5) * 0.2;
                    const vz = (Math.random() - 0.5) * 0.1;

                    // カラーリング：質量が大きいものは黄色〜オレンジ、軽いデブリは青白い淡色
                    let color = "#88ccff";
                    if (mass > 50.0) color = "#ffaa00";
                    else if (mass > 2.0) color = "#ffea88";
                    else if (mass < 0.05) color = "rgba(130, 200, 255, 0.4)"; // デブリは半透明
            ...
            if (window.UI_DEBUG) console.log("🛸 [PRST: 3] 200基のべき乗質量・潮汐破壊スイングバイが正常に執行されました。");
        }
    },


    // 🍕 4. ケプラー・ピザ極限観察（特級数理空間）
    preset4: {
        name: "PRST: 4 (ケプラー数理観察・面積速度一定)",
        ui: {
            bodyCount: 1,         bodyCountInput: 1,
            sunMass: 10000,       sunMassInput: 10000,
            sunVxSlider: 0.0,     sunVxInput: 0.0,
            sunVySlider: 0.0,     sunVyInput: 0.0,
            sunVzSlider: 0.0,     sunVzInput: 0.0,
            objMass: 1.0,         objMassInput: 1.0,
            useFixedObjMass: true,
            gravitySlider: 1.0,   gravityInput: 1.0,
            spawnVelSlider: 0.3,  spawnVelInput: 0.3,
            speedSlider: 1.5,     speedInput: 1.5,
            eps2Input: 0.0,
            cameraRotateSpeed: 0.0005
        },
        physics: () => {
            ...
            if (window.targetHistory) window.targetHistory = [];

            if (window.settings) {
                settings.sunInitialVx = 0.0; settings.sunInitialVy = 0.0; settings.sunInitialVz = 0.0;
                settings.gravityMultiplier = 1.0;
                settings.spawnVelocityMultiplier = 0.3;
          ...
          // 🎯 修理後：targetOFF に 'toggle-momentum-btn' を完全拘束
const targetON = ['planetTrailBtn'];
const targetOFF = ['cometTrailBtn', 'sunTrailBtn', 'toggleBaryBtn', 'toggle-momentum-btn']; // 💡ここに追加！

targetON.forEach(id => {
    const btn = document.getElementById(id);
    if (btn && !btn.classList.contains('toggle-on')) {
        btn.classList.add('toggle-on', 'active');
        btn.classList.remove('toggle-off');
    }
});
targetOFF.forEach(id => {
    ...
    if (btn) {
        btn.classList.add('toggle-off');
        btn.classList.remove('toggle-on', 'active'); // 💡L-VECの古いアクティブ残像を完全パージ
    }
});

// 👑 L-VEC内部変数の初期化と、ボタン表面のカラーをストイックへ完全調律
window.lvecMode = 0;
...
if (lvecBtn) {
    ...
    lvecBtn.style.color = '#fff';
            ...
            if (window.bodies && window.bodies[0]) {
                ...
                if (b.trail) b.maxTrailPoints = 1000;
            }

            if (window.UI_DEBUG) console.log("🍕 [PRST: 4] 単体ピザ観察マトリクスが静かにスタンバイされました。");
        }
    }
};
...
// ★ 規律修正：グラフィックと衝突判定の整合性を取るため、値を「1.0」またはコメントの推奨値に
// ※1.0にすると、描画されている太陽のサイズと完全に一致した綺麗な衝突判定になる
...
function generateBodies() {
  ...
  for (let i = 0; i < initialBodyCount; i++) {
    ...
    const turb = Math.min(1, 300 / dist);
    const side = Math.random() < 0.1 ? -1 : 1; // 10%の確率で逆回り天体を生成する
    const turbBoost = side === -1 ? 1.4 : 1.0;
...
function renderScene() {
 ...
 // ---------------------------------------------------------
  ...
  if (simulationState.ui.showNames) {
    ...
    for (const obj of sortedBodies) {
            // 👑【既存機能ハイジャック】この天体が、今ちょうど「他の誰かと最接近中」かどうかのフラグをチェック
      // ※物理演算側でobj.b.isEncountering = true を立てておく仕様
      const isEncountering = obj.b.isEncountering || false;

      // 天体のタイプによって描画をスキップする冷徹な防空論理
      // 👑【調律】最接近中(isEncountering)であれば、すべてのモード判定をスルーパスして強制描画する！
      if (!isEncountering) {
        if (mode === 1 && obj.b.type !== "obj") continue;   // 惑星のみモードの時は惑星以外パス
        if (mode === 2 && obj.b.type !== "comet") continue; // 彗星のみモードの時は彗星以外パス
      }
      // mode === 3 または isEncountering === true のときはSunを含む全天体をスルーパスして描画
...
function drawAngularMomentumVectorDirect2D(b, sun, pr) {
 ...
 // =========================================================
  // 🎨 【新・絶対規律】渋いトーンの離心率カラーマッピング ＆ バインド
  // =========================================================
// 🟢 パターン①改：【エンタメ特化型・楕円レンジ分散アライメント】
let eccColor = "#00a2ff"; // デフォルト
if (ecc < 0.08)      eccColor = "#00a2ff"; // 🟦 真円〜微小楕円：まだ安定している青（閾値を0.05➔0.08へ微増）
else if (ecc < 0.25) eccColor = "#00e58b"; // 🟩 軽度楕円：小気味よく混ざるサイバーミントグリーン
else if (ecc < 0.45) eccColor = "#ded000"; // 🟨 中度楕円：最も天体が多いゾーンをこの「黄色」でしっかり受ける
else if (ecc < 0.75) eccColor = "#ff7700"; // 🟧 強度楕円：ここから明確に「歪みが牙を剥いた」とわかるサンセットオレンジ
else                 eccColor = "#ff2a55"; // 🟥 臨界〜無限追放：時空からパージされる臨界のブラッドクリムゾン


  // 👑【電線の繋ぎ込み】
  // 決定した渋いトーンの色を、天体オブジェクト自身へダイレクトに記憶させる！
  b.eccColor = eccColor;
...
// ── 1. 🎯 名指し指定（LOCK）の判定 ＆ 自動選出 ──
  ...
  if (window.selectedTargetName && window.selectedTargetName.toUpperCase() !== "AUTO") {
    const searchTargetUpper = window.selectedTargetName.toUpperCase();
        // ⭕ 大文字・小文字を無視して（大文字同士に揃えて）名前を検索！
    target = bodies.find(b => b.name && b.name.toUpperCase() === searchTargetUpper);
  }

  // ② 名指しが無い、または見つからない場合は厳格スコア（質量×角運動量）で自動選定
  if (!target) {
  ...
  if (!target) return;
  // ── 2. 🌌 【核心】真の近日点(P)・遠日点(A)のリアルタイム判定と固定 ──
  // 現在の太陽との距離 r
  ...
  if (target.lastR !== undefined) {
    // 距離の変化傾向（ approach: 接近中 / recede: 遠ざかり中 ）
    const isApproaching = curR < target.lastR;

    // ① 接近から遠ざかる瞬間に切り替わった ➔ 【真の近日点 P】を更新・ピン留め！
    ...
    // ② 遠ざかりから接近する瞬間に切り替わった ➔ 【真の遠日点 A】を更新・ピン留め！
    if (!target.wasApproaching && isApproaching) {
      target.aphoPoint = {
        x: target.x, y: target.y, z: target.z,
        sunX: sun.x, sunY: sun.y, sunZ: sun.z
  ...
  target.lastR = curR; // 前回フレームの距離を保存
 ...
 // ── 5. 📊 リアルタイム HUD パネル ──
  ...
  if (pLock.visible && targetHistory.length >= 2) {
    ...
    // 📡 パネル枠サイズ（高さ68pxへ拡張し全データを格納）
    ctx.fillStyle = "rgba(0, 15, 8, 0.82)";
    ctx.fillRect(panelX - 2, panelY - 8, 165, 68);
    ctx.strokeStyle = "rgba(0, 255, 136, 0.35)";
    ctx.lineWidth = 1;
    ctx.strokeRect(panelX - 2, panelY - 8, 165, 68);
    ...
    if (target.periPoint) {
      ctx.fillText(`P(${target.periPoint.x.toFixed(0)}, ${target.periPoint.y.toFixed(0)}, ${target.periPoint.z.toFixed(0)})`, panelX, panelY += 10);
    } else {
      ctx.fillText(`P(--, --, --)`, panelX, panelY += 10);
    }

    // ⑥ 遠日点 A(x, y, z) 座標
    ctx.fillStyle = "rgba(0, 213, 255, 0.85)";
    if (target.aphoPoint) {
      ctx.fillText(`A(${target.aphoPoint.x.toFixed(0)}, ${target.aphoPoint.y.toFixed(0)}, ${target.aphoPoint.z.toFixed(0)})`, panelX, panelY += 10);
    } else {
      ctx.fillText(`A(--, --, --)`, panelX, panelY += 10);
    }
  }
